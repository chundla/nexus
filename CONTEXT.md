...
- Claude could stay terminal-backed forever while other **Providers** use structured **Session Surfaces** — resolved (planned): Claude moves to a structured primary **Session Surface** on local **Workspaces**, **Remote Workspaces**, and supported iPhone **Remote Client** paths in one initiative, using a long-lived `claude` CLI child with bidirectional `stream-json` (not one-shot `claude -p` per prompt).
- Claude permission prompts could stay TTY-only or use ad-hoc composer text — resolved (planned): Claude `stream-json` permission events map to shared app-native **Approval Requests** with the same **Controller**-gated approve/deny model as Codex and Pi; normal launch does not rely on silently bypassing permissions.
- Claude continuity could reuse tmux scrollback or one Claude thread per **Workspace** — resolved (planned): one **Session Record** (default or **Named Session**) maps 1:1 to one Claude session id in **Session Record** adapter metadata; relaunch uses `--resume` (or equivalent) on the long-lived process. Legacy terminal-backed **Session Records** do not migrate tmux transcript; structured relaunch starts from adapter continuity or a fresh Claude session.
- Structured Claude could ship as a flat activity feed first and add **Agent Turn** presentation later — resolved (planned): Claude joins Pi/Codex/IBM Bob with **Agent Turn** feed segments (Reasoning, Tools, Final answer) in the same initiative as the `stream-json` runtime, not a deferred presentation-only follow-up.
- Claude could keep a terminal primary **Session Surface** alongside structured mode — resolved (planned): hard cutover only. Claude uses structured primary **Session Surface** on all targets; Nexus removes Claude PTY/tmux primary UI and terminal-only remote routes. Orphan host tmux sessions from the prior era are not auto-cleaned.
- Claude **Provider Health** could stay executable-only after structured cutover — resolved (planned): structured readiness uses a capability probe (long-lived or handshake `claude -p` with `stream-json`); **Provider Health** reports not launchable when the probe fails, not only at first **Session** launch.
- Remote structured Claude could skip tmux and rely only on Claude session files — resolved (planned): remote Claude uses the same **Remote Session Strategy** as remote Pi/Codex—a tmux-backed detached long-lived `stream-json` runner, not terminal attach. **Host Validation** keeps tmux required for hosts that run remote structured Claude.
- Structured Claude could integrate via Claude Agent SDK instead of the CLI — resolved (planned): Nexus integrates only by spawning the `claude` executable with print/stream-json flags; no Agent SDK sidecar in the service.
- Nexus could pass a permissive Claude `--permission-mode` to reduce prompts — resolved (planned): v1 uses `--permission-mode default` on all Nexus-launched Claude runtimes so permission events remain explicit and map to shared **Approval Requests**; `bypassPermissions`, `dontAsk`, and broad auto-accept modes are not product defaults.
- Structured Claude launch could use `--bare` or ignore **Workspace** folder context — resolved (planned): Nexus launches with cwd at the **Workspace** target and workspace-scoped `--add-dir`; `--bare` is not the default (preserve CLAUDE.md, skills, and provider-native workspace discovery users expect from Claude Code).