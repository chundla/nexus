...
struct StructuredSessionFeedProgressiveRevealPolicyTests {
    @Test func visibleActivityRowsRevealsTailFirstInBatches() {
        ...
        #expect(
            StructuredSessionFeedProgressiveRevealPolicy.visibleActivityRows(in: feed, visibleTailRowCount: 0).isEmpty)
        ...
        #expect(
            StructuredSessionFeedProgressiveRevealPolicy.visibleActivityRows(in: feed, visibleTailRowCount: 20).count
                == 20)
    }
    ...
    @Test func thinkingIndicatorVisibleDuringProgressiveReveal() {
        ...
        #expect(
            StructuredSessionFeedProgressiveRevealPolicy.shouldShowThinkingIndicator(in: feed, visibleTailRowCount: 0)
                == true)
        #expect(
