...
struct nexusTests {
    ...
    @Test func exitedClaudeRuntimeUsesProviderModuleExitCopyOverIPC() async throws {
        let workspaceFolderURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceFolderURL, withIntermediateDirectories: true)
        ...
        #expect(exitedScreen.transcript.contains("[Claude exited with status 0]"))
        #expect(exitedScreen.transcript.contains("[Adapter exit status 0]") == false)
    }
    ...
    @MainActor
    @Test func appModelContinuesStreamingPiActivityWhileAnotherIPCClientObservesTheSameSession() async throws {
        let workspaceFolderURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceFolderURL, withIntermediateDirectories: true)
        ...
        let remoteClient = try NexusIPCClient.connect(to: service.listenerEndpoint)
        _ = try await macClient.createWorkspaceGroup(name: "Solo Group")
        ...
        let remoteCollector = SessionScreenCollector()
        let remoteSnapshot = try await remoteClient.getSessionScreenObservationSnapshot(sessionID: session.id)
        await remoteCollector.record(remoteSnapshot.screen)
        let remoteAccumulator = SessionScreenObservationAccumulator(snapshot: remoteSnapshot)
        let remoteObservation = try await remoteClient.observeSessionScreenUpdateEvents(sessionID: session.id) { update in
            do {
                if let screen = try remoteAccumulator.apply(update) {
                    Task {
                        await remoteCollector.record(screen)
                    }
                }
            } catch is SessionScreenObservationGapError {
                Task {
                    guard let snapshot = try? await remoteClient.getSessionScreenObservationSnapshot(sessionID: session.id) else {
                        return
                    }
                    await remoteCollector.record(remoteAccumulator.replace(with: snapshot))
                }
            } catch {
                return
            }
        ...
        defer {
            Task {
                await remoteObservation.cancel()
            }
        }

        try await model.sendInputToFocusedSession("delegate")

        let macCompletedScreen = try await waitForObservedFocusedSessionScreen(model: model) { screen in
            screen.session.id == session.id
                && screen.isAgentTurnInProgress == false
                && screen.activityItems.contains(where: { $0.text == "Pi: Done" })
        }
        let remoteCompletedScreen = try await remoteCollector.waitForScreen { screen in
            screen.session.id == session.id
                && screen.isAgentTurnInProgress == false
                && screen.activityItems.contains(where: { $0.text == "Pi: Done" })
        }

        #expect(macCompletedScreen.activityItems.map(\.text) == [
            "Pi shared Session stream connected",
            "You: delegate",
            "subagent reviewer: Review the latest diff and summarize issues",
            "subagent: Looks good overall. Watch the new error path.",
        ...
        #expect(remoteCompletedScreen.activityItems.map(\.text) == macCompletedScreen.activityItems.map(\.text))
    }
