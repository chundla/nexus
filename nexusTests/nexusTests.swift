...
struct nexusTests {
    ...
    @Test func terminalViewportResizeCoordinatorCoalescesRapidViewportChangesToFinalSize() async throws {
        ...
        try await waitUntil(timeoutNanoseconds: 1_000_000_000, pollIntervalNanoseconds: 1_000_000) {
            submittedSizes == [.init(columns: 120, rows: 36)]
                && currentSize == .init(columns: 120, rows: 36)
        }
    ...
    @MainActor
    @Test func focusedStructuredSessionPresentationUsesTheSharedRootPresentationAndConversationPolicy() {
    ...
    @Test func existingDefaultSessionKeepsLaunchCapabilityEnabledWhenProviderHealthBecomesUnavailable() async throws {
        ...
        let overview = try await client.refreshWorkspaceOverview(workspaceID: workspace.id)
        let claudeCard = try #require(overview.providerCards.first(where: { $0.provider.id == .claude }))
    ...
    @Test func workspaceOverviewShowsAllSupportedProvidersOverIPC() async throws {
        ...
        #expect(
            overview.providerCards.map(\.health.state) == [.notChecked, .notChecked, .notChecked, .notChecked])
    ...
    @Test func launchOrResumeDefaultSessionUsesClaudeProviderModuleHealthOverIPC() async throws {
        let workspaceFolderURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceFolderURL, withIntermediateDirectories: true)

        let resolvedExecutable = "/tmp/provider-module-claude"
        let service = try NexusService.bootstrapForTests(
            rootURL: FileManager.default.temporaryDirectory
                .appendingPathComponent("NexusTests", isDirectory: true)
                .appendingPathComponent(UUID().uuidString, isDirectory: true),
        ...
        #expect(session.state == .ready)
        #expect(screen.transcript == resolvedExecutable)
        #expect(claudeCard.health.summary == "Claude 9.9.9 (Claude Code) is available")
        #expect(claudeCard.health.resolvedExecutable == resolvedExecutable)
    }

    @Test func launchOrResumeDefaultSessionUsesClaudeProviderModuleLaunchCopyOverIPC() async throws {
        let workspaceFolderURL = FileManager.default.temporaryDirectory
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try FileManager.default.createDirectory(at: workspaceFolderURL, withIntermediateDirectories: true)
        ...
        #expect(session.state == .ready)
        #expect(screen.transcript == "Launching \(workspace.name) with Claude…\n")
    }

    @Test func createNamedSessionUsesClaudeProviderModuleHealthOnRemoteWorkspaceOverIPC() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("NexusTests", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let availabilityRunner = StubCommandRunner(results: [
    ...
    @Test func recoveredRemoteClaudeSessionUsesProviderModuleReconnectCopyOverIPC() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("NexusTests", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let availabilityRunner = StubCommandRunner(results: [
        ...
        #expect(recoveredSession.id == launchedSession.id)
        #expect(recoveredScreen.transcript == "Reconnecting to \(workspace.name) on Build Server with Claude…\n")
    }
    ...
    @Test func missingRecoveredRemoteRuntimeUsesClaudeProviderModuleRecoveryFailureOverIPC() async throws {
        let rootURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("NexusTests", isDirectory: true)
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        let availabilityRunner = StubCommandRunner(results: [
    ...
    @MainActor
    @Test func appModelWorkspaceGroupDetailPresentationStaysStableDuringProviderDetailLoads() async throws {
        let group = WorkspaceGroup(id: UUID(), name: "Group")
        let workspace = Workspace(
            id: UUID(),
            name: "Workspace",
        ...
        await model.refresh()
        let initialPresentation = model.workspaceGroupDetailPresentation(groupID: group.id)
        ...
        let presentationChanged = ObservationChangeState()
        withObservationTracking {
            _ = model.workspaceGroupDetailPresentation(groupID: group.id)
        } onChange: {
            Task { @MainActor in
                presentationChanged.changed = true
            }
        ...
        #expect(presentationChanged.changed == false)
        #expect(model.workspaceGroupDetailPresentation(groupID: group.id) == initialPresentation)
    }
    ...
    @MainActor
    @Test func appModelFocusedStructuredSessionPresentationStaysStableDuringChromeOnlyExtensionUpdates() async throws {
        let group = WorkspaceGroup(id: UUID(), name: "Group")
        let workspace = Workspace(
            id: UUID(),
            name: "Workspace",
        ...
        try await waitUntilAsync {
            await MainActor.run {
                model.workspaceOverview(for: workspace.id)?.providerCards.first(where: { $0.provider.id == .claude })?
                    .defaultSession.state == .ready
                    && model.workspaceOverview(for: workspace.id)?.providerCards.first(where: {
                        $0.provider.id == .codex
                    })?.defaultSession.state == .ready
                    && model.workspaceOverview(for: workspace.id)?.providerCards.first(where: {
                        $0.provider.id == .pi
                    })?.defaultSession.state == .ready
                    && model.providerDetail(for: workspace.id, providerID: .claude)?.defaultSession?.id
                        == claudeSession.id
                    && model.providerDetail(for: workspace.id, providerID: .codex)?.defaultSession?.id
                        == codexSession.id
                    && model.providerDetail(for: workspace.id, providerID: .pi)?.defaultSession?.id
                        == piSession.id
            }
        }
    ...
    @MainActor
    @Test func appModelLaunchOrResumeDefaultSessionDoesNotAwaitRefreshFollowUps() async throws {
        let group = WorkspaceGroup(id: UUID(), name: "Group")
        let workspace = Workspace(
            id: UUID(),
            name: "Workspace",
        ...
        let staleDetail = ProviderDetail(
            workspace: workspace,
            provider: Provider(id: .claude),
            health: ProviderHealthSummary(state: .available, summary: "Claude available"),
            defaultSession: nil,
        ...
        let freshDetail = ProviderDetail(
            workspace: workspace,
            provider: Provider(id: .claude),
            health: ProviderHealthSummary(state: .available, summary: "Claude available"),
        ...
        try await waitUntil { launchedSession != nil }
        try await waitUntil {
            client.refreshWorkspaceOverviewRequestCount == 1 && client.providerDetailRequestCount == 2
        }

        #expect(launchedSession?.id == defaultSession.id)
        #expect(model.focusedSessionScreen?.session.id == defaultSession.id)
        #expect(model.providerDetail(for: workspace.id, providerID: .claude)?.defaultSession == nil)

        await workspaceOverviewGate.resume(returning: freshOverview)
        await providerDetailGate.resume(returning: freshDetail)

        try await waitUntilAsync {
            await MainActor.run {
                model.workspaceOverview(for: workspace.id)?.providerCards.first?.defaultSession.sessionID
                    == defaultSession.id
                    && model.providerDetail(for: workspace.id, providerID: .claude)?.defaultSession?.id
                        == defaultSession.id
            }
    ...
    @MainActor
    @Test func appModelRefreshesFocusedSessionAfterObservationAdvancesBeforeActionResponseReturns() async throws {
        actor ReturnGate {
            private var continuation: CheckedContinuation<Void, Never>?
        ...
        let observedThinkingScreen = SessionScreen(
            session: session,
            primarySurface: .structuredActivityFeed,
            transcript: "",
            activityItems: [
        ...
        let finalActionResponseScreen = SessionScreen(
            session: session,
            primarySurface: .structuredActivityFeed,
            transcript: "",
            activityItems: [
        ...
        client.getSessionScreenHandler = { _ in
            finalActionResponseScreen
        }
        let returnGate = ReturnGate()
        client.sendSessionInputHandler = { [weak client] _, _ in
            guard let client else {
                return finalActionResponseScreen
            }

            await client.emitObservedScreen(observedThinkingScreen)
            await returnGate.wait()
            return finalActionResponseScreen
        }
        let model = NexusAppModel(client: client)
        ...
        let observedThinking = try await waitForObservedFocusedSessionScreen(model: model) { screen in
            screen == observedThinkingScreen
        }
        #expect(observedThinking.isAgentTurnInProgress)

        await returnGate.open()
        try await sendTask.value

        #expect(model.focusedSessionScreen == finalActionResponseScreen)
        #expect(model.focusedSessionScreen?.isAgentTurnInProgress == false)
    }
...
private actor SuspendedResultGate<T: Sendable> {
...
private final class TrackingServiceClient: NexusServiceClient, @unchecked Sendable {
    ...
    private var refreshWorkspaceOverviewHandler: (@Sendable (UUID) async throws -> WorkspaceOverview)?
    private var providerDetailHandler: (@Sendable (UUID, ProviderID) async throws -> ProviderDetail)?
    ...
    init(
        ...
        refreshWorkspaceOverviewHandler: (@Sendable (UUID) async throws -> WorkspaceOverview)? = nil,
        providerDetailHandler: (@Sendable (UUID, ProviderID) async throws -> ProviderDetail)? = nil,
        ...
        self.refreshWorkspaceOverviewHandler = refreshWorkspaceOverviewHandler
        self.providerDetailHandler = providerDetailHandler
        self.structuredHistoryPages = structuredHistoryPages
    }

    func setRefreshWorkspaceOverviewHandler(
        _ handler: (@Sendable (UUID) async throws -> WorkspaceOverview)?
    ) {
        refreshWorkspaceOverviewHandler = handler
    }

    func setProviderDetailHandler(
        _ handler: (@Sendable (UUID, ProviderID) async throws -> ProviderDetail)?
    ) {
        providerDetailHandler = handler
    }
    ...
    func refreshWorkspaceOverview(workspaceID: UUID) async throws -> WorkspaceOverview {
        ...
        let refreshed =
            try await refreshWorkspaceOverviewHandler?(workspaceID)
    ...
    func getProviderDetail(workspaceID: UUID, providerID: ProviderID) async throws -> ProviderDetail {
        ...
        let detail = try await providerDetailHandler?(workspaceID, providerID) ?? providerDetailValue
        providerDetailValue = detail
        return detail
    }
