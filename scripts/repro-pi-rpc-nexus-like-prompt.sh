#!/usr/bin/env bash
# Reproduces Nexus local Pi RPC startup + model switch + user prompt (no --session).
set -euo pipefail
...
python3 - "$PI" "$CWD" "$USER_MESSAGE" "$MODEL_PROVIDER" "$MODEL_ID" "$POST_TOOL_STALL_SEC" "$OVERALL_TIMEOUT_SEC" <<'PY'
import json, os, select, subprocess, sys, time
from datetime import datetime, timezone

pi, cwd, user_message, model_provider, model_id, post_tool_stall_sec, overall_timeout_sec = sys.argv[1:8]
    ...
    while set_model_id not in responses and time.monotonic() < start + 30:
        ...
        if obj.get("type") == "response":
            ...
            if obj.get("command") == "set_model" and obj.get("id") == set_model_id:
                break
    set_resp = responses.get(set_model_id)
    if set_resp and set_resp.get("success") is not True:
        print(f"[{ts()}] WARN set_model failed: {set_resp}", flush=True)
else:
    print(f"[{ts()}] skip set_model (prompt-only)", flush=True)
    ...
    if not r:
        ...
        if saw_tool_results >= 3 and not saw_agent_end and idle >= post_tool_stall_sec and not stall_reported:
            ...
            print(
                ...
                f"agent_end={saw_agent_end} prompt_accept={saw_prompt_accept}",
                flush=True,
            )
        continue
    line = proc.stdout.readline()
    ...
    if obj.get("type") == "response":
        ...
        if obj.get("command") == "prompt" and obj.get("id") == prompt_id:
            if obj.get("success") is not True:
                print(f"[{ts()}] FAIL prompt rejected: {obj}", flush=True)
                proc.kill()
                sys.exit(3)
...
print(f"[{ts()}] SUMMARY elapsed={time.monotonic()-start:.1f}s prompt_accept={saw_prompt_accept} "
      f"toolResults={saw_tool_results} agent_end={saw_agent_end} last_assistant_stop={last_assistant_stop}", flush=True)
