    ...
    final class NexusAppModel {
        ...
        nonisolated static func appBootstrapListeningPort(
        ...
        func startPairing() async throws -> PairingCeremony {
            ...
            remoteAccessState = RemoteAccessState(
                isEnabled: remoteAccessState?.isEnabled ?? true, activePairing: pairing)
            return pairing
        }
        ...
        func relaunchFocusedSession() async throws -> Session {
            guard let screen = focusedSessionScreen else {
                throw NSError(
                    domain: "NexusAppModel", code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "No focused session to relaunch"])
            }
        ...
        func respondToFocusedSessionApprovalRequest(_ approvalRequestID: UUID, decision: ApprovalRequestDecision)
            async throws
        {
            guard let baselineScreen = focusedSessionScreen else {
                return
        ...
        func respondToFocusedSessionExtensionDialog(_ dialogID: String, response: SessionExtensionUIDialogResponse)
            async throws
        {
            guard let baselineScreen = focusedSessionScreen else {
                return
        ...
        func resizeFocusedSession(columns: Int, rows: Int) async throws {
            ...
            let screen = try await client.resizeSession(
                sessionID: baselineScreen.session.id, columns: columns, rows: rows)
            try await applyFocusedSessionActionResponse(screen, ifCurrentScreenMatches: baselineScreen)
        }
        ...
        func workspaceBrowseNavigationPresentation(currentWorkspaceID: UUID?) -> WorkspaceBrowseNavigationPresentation {
            ...
            let initialSelection =
                ...
                ?? sidebarPresentation.workspaces.first.map {
                    WorkspaceBrowseInitialSelection.workspace($0.workspace.id)
                }
                ?? sidebarPresentation.workspaceGroups.first.map {
                    WorkspaceBrowseInitialSelection.workspaceGroup($0.group.id)
                }
        ...
        private func applyWorkspaceOverviews(
            ...
            let applicableOverviews = overviews.filter {
                shouldApplyWorkspaceOverview($0, refreshGeneration: refreshGeneration)
            }
        ...
        private func syncStaleWorkspaceOverviewRefreshTask(for overview: WorkspaceOverview) {
            ...
            staleWorkspaceOverviewRefreshTasks[workspaceID] = Task { @MainActor [weak self] in
                ...
                guard let refreshedOverview = try? await client.refreshWorkspaceOverview(workspaceID: workspaceID)
                else {
                    return
                }
        ...
        private func cancelStaleWorkspaceOverviewRefreshTasks(excluding workspaceIDs: Set<UUID>) {
            for (workspaceID, task) in staleWorkspaceOverviewRefreshTasks
            where workspaceIDs.contains(workspaceID) == false {
                task.cancel()
                staleWorkspaceOverviewRefreshTasks.removeValue(forKey: workspaceID)
            }
        }
        ...
        private func syncRecentNavigationSessionWorkspaceIDs() {
            ...
            recentNavigationSessionWorkspaceIDs = recentNavigationSessionWorkspaceIDs.filter {
            ...
            for overview in workspaceOverviews.values {
                for sessionID in overview.providerCards.compactMap(\.defaultSession.sessionID)
                where sessionIDs.contains(sessionID) {
                    recentNavigationSessionWorkspaceIDs[sessionID] = overview.workspace.id
                }
            }

            for detail in providerDetails.values {
                let sessionIDsInDetail =
                    [detail.defaultSession].compactMap { $0 } + detail.alternateSessions + detail.failedSessions
        ...
        private func syncRecentNavigationSessionWorkspaceIDs(for overview: WorkspaceOverview) {
            ...
            for sessionID in overview.providerCards.compactMap(\.defaultSession.sessionID)
            where relevantSessionIDs.contains(sessionID) {
                recentNavigationSessionWorkspaceIDs[sessionID] = overview.workspace.id
            }
        }
        ...
        private func applyFocusedSessionActionResponse(
            ...
            guard let currentScreen = focusedSessionScreen,
                currentScreen.session.id == baselineScreen.session.id
            else {
                return
            }
