    ...
    extension NexusAppModel {
        static func bootstrap(environment: [String: String] = ProcessInfo.processInfo.environment) throws
            -> NexusAppModel
        {
        ...
        private actor FixtureServiceClient: NexusServiceClient {
            ...
            func validateHost(hostID: UUID) async throws -> HostValidationSnapshot {
                HostValidationSnapshot(
                    hostID: host.id, state: .available, summary: "Fixture host ready",
                    checkedAt: Date(timeIntervalSince1970: 600))
            }
            ...
            func listRecentNavigation(limit: Int) async throws -> [NavigationItem] {
                [
                    NavigationItem(
                        target: .session(session.id), title: "Structured Feed Profile", subtitle: workspace.folderPath),
                    NavigationItem(
                        target: .workspace(workspace.id), title: workspace.name, subtitle: workspace.folderPath),
                ]
            }
            ...
            func createLocalWorkspace(name: String?, folderPath: String, primaryGroupID: UUID?) async throws
                -> Workspace
            { try unsupported() }
            func createRemoteWorkspace(name: String?, hostID: UUID, remotePath: String, primaryGroupID: UUID?)
            ...
            func getStructuredSessionHistoryPage(
                ...
                StructuredSessionHistoryPage(
                    sessionID: session.id, activityItems: [], providerEvents: [], nextCursor: nil)
            }
            ...
            func sendRemoteSessionInput(sessionID: UUID, pairedDeviceID: UUID, text: String) async throws
                -> SessionScreen
            { try unsupported() }
            ...
            ) async throws -> SessionScreen { try unsupported() }
            ...
            func sendRemoteSessionText(sessionID: UUID, pairedDeviceID: UUID, text: String) async throws
                -> SessionScreen
            { try unsupported() }
                ...
                throw NSError(
                    ...
                    userInfo: [
                        NSLocalizedDescriptionKey:
                            "This macOS profiling fixture only supports deterministic structured feed profiling flows."
                    ]
                )
            }
        }
