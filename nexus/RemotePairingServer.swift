    ...
    nonisolated final class RemotePairingServer: RemotePairingServing, @unchecked Sendable {
        ...
        init(
            ...
            if let listeningPort {
                guard (1...65_535).contains(listeningPort),
                    let nwPort = NWEndpoint.Port(rawValue: UInt16(listeningPort))
                else {
        ...
        private func receive(on connection: NWConnection, accumulated: Data) {
            connection.receive(minimumIncompleteLength: 1, maximumLength: 65_536) { [weak self] data, _, isComplete, error in
                ...
                do {
                    ...
                    send(
                        statusCode: 400, body: RemotePairingErrorResponse(message: error.localizedDescription),
                        over: connection)
                    return
                }
                ...
                if let error {
                    send(
                        statusCode: 500, body: RemotePairingErrorResponse(message: error.localizedDescription),
                        over: connection)
                    return
                }
                ...
                if isComplete {
                    send(
                        statusCode: 400, body: RemotePairingErrorResponse(message: "Incomplete request"),
                        over: connection)
                    return
                }
        ...
        private func respond(to request: ParsedRequest, over connection: NWConnection) async {
            ...
            if request.method == "GET", request.path == "/remote-client/catalog" {
                await respondToAuthorizedRequest(
                    ...
                    let workspaceOverviews = try await self.fetchCatalogWorkspaceOverviewsSummaryFirst(
                ...
                let sessionScreenObservationRequest = sessionScreenObservationRequest(from: request)
            {
                do {
                    ...
                    let observation = try await client.observeSessionScreenUpdateEvents(
                    ...
                    if let expectedStructuredRevision {
                        let latestSnapshot = try await client.getSessionScreenObservationSnapshot(
                            sessionID: sessionScreenObservationRequest.sessionID)
                    ...
                    send(
                        statusCode: 401, body: RemotePairingErrorResponse(message: Self.revokedPairingMessage),
                        over: connection)
                } catch {
                    await recordRemoteClientDiagnosticBreadcrumb(
                        kind: .reconnectFailure,
            ...
            do {
                ...
                let pairedDevice = try await client.completePairing(
                    pairingCode: pairingRequest.pairingCode, deviceName: pairingRequest.deviceName)
                send(
                    statusCode: 200,
        ...
        private func pairedDeviceID(from request: ParsedRequest) throws -> UUID {
            guard let rawValue = request.headers["x-nexus-paired-device-id"],
                let pairedDeviceID = UUID(uuidString: rawValue)
            else {
                throw RemotePairingServerError.unauthorized
            }
        ...
        private func sessionScreenObservationSnapshotRequest(from request: ParsedRequest)
            -> SessionScreenObservationRequest?
        {
            let components = request.path.split(separator: "/")
        ...
        private func structuredSessionHistoryPageRequest(from request: ParsedRequest)
            -> StructuredSessionHistoryPageRequest?
        {
            ...
            guard pathComponents.count == 4,
                ...
                pathComponents[3] == "structured-history"
            else {
                return nil
            }
            ...
            let pageSize =
                queryItems.first(where: { $0.name == "pageSize" }).flatMap { $0.value }.flatMap(Int.init) ?? 50
            let activityItemOffset = queryItems.first(where: { $0.name == "activityItemOffset" }).flatMap { $0.value }
                .flatMap(Int.init)
            let providerEventOffset = queryItems.first(where: { $0.name == "providerEventOffset" }).flatMap { $0.value }
                .flatMap(Int.init)
            let cursor: StructuredSessionHistoryCursor? =
                if let activityItemOffset, let providerEventOffset {
        ...
        private func structuredSessionArtifactFileRequest(from request: ParsedRequest)
            -> StructuredSessionArtifactFileRequest?
        {
            guard let components = URLComponents(string: "http://localhost\(request.path)") else {
                return nil
            }
        ...
        private func extensionDialogResponseRequest(from request: ParsedRequest)
            -> SessionExtensionDialogResponseRequest?
        {
            let components = request.path.split(separator: "/")
            guard components.count == 6,
                components[0] == "remote-client",
    ...
    nonisolated private final class RemoteSessionScreenStream: @unchecked Sendable {
        ...
        private func sendHeaders() {
            let responseHead =
                ...
                + "\r\n"
            connection.send(
                content: Data(responseHead.utf8),
