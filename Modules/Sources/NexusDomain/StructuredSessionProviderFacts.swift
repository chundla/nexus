...
public struct StructuredSessionProviderFacts: Codable, Equatable, Sendable {
    ...
    private static func modelIdentifier(
        ...
        case .pi:
            guard let data = payload["data"] as? [String: Any],
                  ...
                  let modelID = trimmedString(in: model, keys: ["id"]) else {
                return nil
            }
    ...
    private static func tokenUsage(from payload: [String: Any]?) -> StructuredSessionProviderTokenUsage? {
        guard let payload,
              let usage = resolvedTokenUsage(from: payload) else {
            return nil
        }
        return usage
    }
    ...
    private static func directTokenUsage(from object: [String: Any]) -> StructuredSessionProviderTokenUsage? {
        let totalTokens = intValue(in: object, keys: ["contextWindow", "context_window", "maxTokens", "max_tokens", "totalTokens", "total_tokens"])
        let explicitUsedTokens = intValue(in: object, keys: ["tokens", "usedTokens", "used_tokens", "tokenCount", "token_count"])
            ?? summedIntValue(in: object, keyPairs: [("inputTokens", "outputTokens"), ("input_tokens", "output_tokens")])
        ...
        let usedTokens = explicitUsedTokens
    ...
    private static func summedIntValue(
        ...
        for (lhsKey, rhsKey) in keyPairs {
            guard let lhs = intValue(in: object, keys: [lhsKey]),
                  let rhs = intValue(in: object, keys: [rhsKey]) else {
                continue
            }
            return lhs + rhs
        }
