    ...
    @objc public protocol NexusXPCProtocol {
        ...
        func getStructuredSessionHistoryPage(
            sessionID: String, pageSize: Int, cursorPayload: Data, reply: @escaping (Data?, NSString?) -> Void)
        func getStructuredSessionArtifactFile(
            sessionID: String, hostPath: String, pairedDeviceID: String?, reply: @escaping (Data?, NSString?) -> Void)
    ...
    public protocol NexusServiceClient: Sendable {
        ...
        func respondToRemoteExtensionDialog(
            sessionID: UUID, pairedDeviceID: UUID, dialogID: String, response: SessionExtensionUIDialogResponse
        ) async throws -> SessionScreen
    ...
    extension NexusServiceClient {
        public func sendSessionInput(sessionID: UUID, prompt: SessionPrompt) async throws -> SessionScreen {
        ...
        public func respondToExtensionDialog(
            sessionID: UUID, dialogID: String, response: SessionExtensionUIDialogResponse
        ) async throws -> SessionScreen {
            _ = sessionID
            _ = dialogID
            _ = response
        ...
        public func respondToRemoteExtensionDialog(
            sessionID: UUID,
            pairedDeviceID: UUID,
            dialogID: String,
        ...
        public func sendRemoteSessionInput(sessionID: UUID, pairedDeviceID: UUID, prompt: SessionPrompt) async throws
            -> SessionScreen
        {
            if prompt.images.isEmpty == false {
                throw NSError(
                    ...
                    code: 1,
                    userInfo: [
                        NSLocalizedDescriptionKey: "This client does not support image-bearing remote Session prompts."
                    ]
                )
            }
    ...
    public protocol SessionScreenObservationEventClient: NexusServiceClient {
        func getSessionScreenObservationSnapshot(sessionID: UUID) async throws
            -> SessionScreenObservationSnapshotResponse
        func getStructuredSessionHistoryPage(
            sessionID: UUID,
            pageSize: Int,
            before cursor: StructuredSessionHistoryCursor?
    ...
    public final class NexusIPCClient: NexusServiceClient, SessionScreenObservationEventClient, @unchecked Sendable {
        ...
        nonisolated public func updateHost(hostID: UUID, name: String, sshTarget: String, port: Int?) async throws
            -> NexusDomain.Host
        {
            try await requestDecodable { proxy, reply in
                proxy.updateHost(
        ...
        nonisolated public func recordRemoteClientDiagnosticBreadcrumb(_ breadcrumb: RemoteClientDiagnosticBreadcrumb)
            async throws
        {
        ...
        nonisolated public func listPerformanceDiagnostics(limit: Int = 10) async throws
        ...
        nonisolated public func getProviderDetail(workspaceID: UUID, providerID: ProviderID) async throws
            -> ProviderDetail
        {
            try await requestDecodable { proxy, reply in
        ...
        nonisolated public func createLocalWorkspace(name: String?, folderPath: String, primaryGroupID: UUID?)
        ...
        nonisolated public func createRemoteWorkspace(
            name: String?, hostID: UUID, remotePath: String, primaryGroupID: UUID?
        ) async throws -> Workspace {
            try await requestDecodable { proxy, reply in
        ...
        nonisolated public func launchOrResumeDefaultSession(workspaceID: UUID, providerID: ProviderID) async throws
            -> Session
        {
            try await requestDecodable { proxy, reply in
        ...
        nonisolated public func createNamedSession(workspaceID: UUID, providerID: ProviderID, name: String?)
        ...
        nonisolated public func observeSessionScreen(
            ...
            observationClient.sessionScreenObserverBridge.registerHandler(
                { update in
                    do {
                        ...
                        Task {
                            guard
                                let latestSnapshot = try? await observationClient.getSessionScreenObservationSnapshot(
                                    sessionID: sessionID)
                            else {
                                return
                            }
        ...
        nonisolated public func takeRemoteSessionControl(sessionID: UUID, pairedDeviceID: UUID, columns: Int, rows: Int)
            async throws -> SessionScreen
        {
            try await requestDecodable { proxy, reply in
        ...
        nonisolated public func releaseRemoteSessionControl(sessionID: UUID, pairedDeviceID: UUID) async throws
            -> SessionScreen
        {
            try await requestDecodable { proxy, reply in
        ...
        nonisolated public func sendRemoteSessionInput(sessionID: UUID, pairedDeviceID: UUID, text: String) async throws
            -> SessionScreen
        {
            try await requestDecodable { proxy, reply in
        ...
        nonisolated public func sendRemoteSessionInput(sessionID: UUID, pairedDeviceID: UUID, prompt: SessionPrompt)
            async throws -> SessionScreen
        {
            let payload = try JSONEncoder().encode(prompt)
            return try await requestDecodable { proxy, reply in
        ...
        nonisolated public func sendRemoteSessionText(sessionID: UUID, pairedDeviceID: UUID, text: String) async throws
            -> SessionScreen
        {
            try await requestDecodable { proxy, reply in
        ...
        nonisolated public func sendRemoteSessionInputKey(sessionID: UUID, pairedDeviceID: UUID, key: SessionInputKey)
            async throws -> SessionScreen
        {
            try await requestDecodable { proxy, reply in
        ...
        private nonisolated func makeObservationHandle(
            ...
            NexusSessionScreenObservationHandle(
                ...
                let _: Bool =
                    (try? await self.requestDecodable { proxy, reply in
    ...
    final class NexusSessionScreenObserverBridge: NSObject, NexusSessionScreenObserverXPCProtocol, @unchecked Sendable {
        private final class HandlerRegistration: @unchecked Sendable {
            ...
            func deliver(_ payload: Data) {
                queue.async { [handler] in
                    guard let update = try? JSONDecoder().decode(SessionScreenObservationUpdate.self, from: payload)
                    else {
                        return
                    }
                    handler(update)
        ...
        func registerHandler(
            _ handler: @escaping @Sendable (SessionScreenObservationUpdate) -> Void, for observationID: UUID
        ) {
            lock.lock()
