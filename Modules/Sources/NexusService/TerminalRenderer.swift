...
enum TerminalRenderer {
    static func renderState(
        ...
        func scrollUpWithinRegion(_ count: Int) {
            ...
            wrappedLineContinuations.insert(contentsOf: Array(repeating: false, count: scrollCount), at: region.upperBound - scrollCount + 1)
        }
        ...
        func scrollDownWithinRegion(_ count: Int) {
            ...
            wrappedLineContinuations.insert(contentsOf: Array(repeating: false, count: scrollCount), at: region.lowerBound)
        }
        ...
        func insertBlankLinesWithinRegion(_ count: Int, at lineIndex: Int) {
            ...
            let replacement = Array(repeating: [TerminalCell](), count: insertCount) + Array(regionSlice.dropLast(insertCount))
            let continuationReplacement = Array(repeating: false, count: insertCount) + Array(continuationSlice.dropLast(insertCount))
        ...
        func deleteLinesWithinRegion(_ count: Int, at lineIndex: Int) {
            ...
            let regionSlice = Array(lines[lineIndex...region.upperBound])
            let continuationSlice = Array(wrappedLineContinuations[lineIndex...region.upperBound])
            let replacement = Array(regionSlice.dropFirst(deleteCount)) + Array(repeating: [TerminalCell](), count: deleteCount)
            let continuationReplacement = Array(continuationSlice.dropFirst(deleteCount)) + Array(repeating: false, count: deleteCount)
            lines.replaceSubrange(lineIndex...region.upperBound, with: replacement)
            wrappedLineContinuations.replaceSubrange(lineIndex...region.upperBound, with: continuationReplacement)
        }
        ...
        func parseCSI(finalCharacter: UnicodeScalar, parameters: String) {
            ...
            case "L":
                ...
                if activeScrollRegion()?.contains(cursorLine) == true {
                    ...
                    wrappedLineContinuations.insert(contentsOf: Array(repeating: false, count: insertCount), at: cursorLine)
                }
            case "M":
                pendingAutoWrap = false
                ensureCurrentLine()
            ...
            case "T":
                ...
                if activeScrollRegion() != nil {
                    ...
                    if scrollCount >= visibleLineCount {
                        ...
                        wrappedLineContinuations.insert(contentsOf: Array(repeating: false, count: scrollCount), at: 0)
    ...
    private static func makeViewport(
        ...
        terminalColumns: Int,
        terminalRows: Int
    ) -> (transcript: String, visibleLines: [String], styledVisibleLines: [TerminalLine], cursorRow: Int, cursorColumn: Int) {
        ...
        let transcript = sourceLines.enumerated().reduce(into: "") { partialResult, entry in
            ...
            let isContinuation = wrappedLineContinuations.indices.contains(lineIndex) ? wrappedLineContinuations[lineIndex] : false
