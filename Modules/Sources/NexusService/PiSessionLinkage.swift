    ...
    extension SessionRecordAdapterMetadata {
        static func pi(
            ...
            encode(
                compactedPiMetadataActivityItems(activityItems), key: PiSessionRecordMetadataKey.activityItemsJSON,
                into: &values)
        ...
        var piPersistedExtensionUIState: SessionExtensionUIState? {
            guard
                let state: SessionExtensionUIState = decode(
                    SessionExtensionUIState.self, key: PiSessionRecordMetadataKey.extensionUIStateJSON),
                piExtensionUIStateHasContent(state)
            else {
                return nil
            }
            return state
    ...
    private func compactedPiMetadataActivityItems(_ activityItems: [SessionActivityItem]) -> [SessionActivityItem] {
        guard encodedJSONCharacterCount(activityItems) > PiMetadataActivityItemCompaction.maxEncodedJSONCharacters
        else {
    ...
    private func truncatedPiMetadataActivityItem(_ item: SessionActivityItem) -> SessionActivityItem {
        SessionActivityItem(
            ...
            text: item.text,
            detailText: nil,
            prompt: compactedPiMetadataPrompt(
                item.prompt, limit: PiMetadataActivityItemCompaction.maxEncodedJSONCharacters / 4)
        )
    }

    private func compactedPiMetadataPrompt(
        _ prompt: SessionPrompt?, limit: Int = PiMetadataActivityItemCompaction.maxPerPromptTextCharacters
    ) -> SessionPrompt? {
