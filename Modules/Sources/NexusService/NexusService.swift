    ...
    protocol SessionRuntimeManaging: AnyObject {
        ...
        func launchOrResume(
            session: Session, workspace: Workspace, launchConfiguration: SessionRuntimeLaunchConfiguration) async throws
        ...
        func sendInputKey(_ key: SessionInputKey, applicationCursorMode: Bool, to session: Session) throws
            -> SessionScreen
        ...
        func sendInput(_ prompt: SessionPrompt, to session: Session) throws -> SessionScreen {
            if prompt.images.isEmpty == false {
                throw NSError(
                    ...
                    code: 1,
                    userInfo: [
                        NSLocalizedDescriptionKey: "This Session runtime does not support image-bearing prompts."
                    ]
                )
            }
    ...
    final class SessionControllerRegistry: @unchecked Sendable {
        ...
        func claimMacControl(sessionID: UUID, preferredSize: (columns: Int, rows: Int)? = nil) -> (
    ...
    struct RemoteSessionCommandBuilder {
        func launchArguments(configuration: SessionRuntimeLaunchConfiguration) -> [String] {
            guard let host = configuration.remoteHost,
                let runtimeIdentifier = configuration.remoteRuntimeIdentifier
            else {
                return []
            }
    ...
    final class ProcessSessionRuntimeLauncher: SessionRuntimeLaunching {
        ...
        init(
            ...
            self.localShellEnvironmentResolver =
                localShellEnvironmentResolver
        ...
        private func makeRemoteTerminalRuntime(
            ...
            guard let remoteHost = launchConfiguration.remoteHost,
                ...
                throw NSError(
                    ...
                    code: 1,
                    userInfo: [
                        NSLocalizedDescriptionKey: "Remote terminal launch requires a Host and runtime identifier."
                    ]
                )
            }
            ...
            return try ProcessSessionRuntime(
                ...
                stopHandler: {
                    try Self.runCommand(
                        executable: "/usr/bin/ssh",
                        arguments: self.remoteSessionCommandBuilder.stopArguments(
                            runtimeIdentifier: runtimeIdentifier, host: remoteHost)
                    )
                },
        ...
        private func makeRemoteIBMBobRuntime(
            ...
            guard let remoteHost = launchConfiguration.remoteHost,
                ...
                throw NSError(
                    ...
                    code: 1,
                    userInfo: [
                        NSLocalizedDescriptionKey: "Remote IBM Bob launch requires a Host and runtime identifier."
                    ]
                )
            }
        ...
        static func runCommand(executable: String, arguments: [String]) throws {
            ...
            guard process.terminationStatus == 0 else {
                ...
                throw NSError(
                    domain: "ProcessSessionRuntimeLauncher", code: Int(process.terminationStatus),
    ...
    struct RemoteRuntimeRecoveryFailureContext {
        ...
        var isMissingRemoteRuntime: Bool {
            normalizedDetail.contains("nexus_remote_runtime_not_found")
                || normalizedDetail.contains("can't find session")
        }
    }
    ...
    public final class NexusService: NSObject, NexusEmbeddedServiceSession, @unchecked Sendable {
        ...
        private init(
            ...
            self.sessionRecordStore =
                sessionRecordStore ?? MetadataStoreSessionRecordStore(metadataStore: metadataStore)
            ...
            self.piStructuredSessionHistoryStore =
            ...
            self.workspaceCatalog = WorkspaceCatalog(
                dependencies: WorkspaceCatalogDependencies(
                    ...
                    sessionRecordStore: sessionRecordStore
                        ?? MetadataStoreSessionRecordStore(metadataStore: metadataStore),
            ...
            self.sessionInteraction =
                sessionInteraction
                ?? ServiceSessionInteraction(
                    dependencies: ServiceSessionInteractionDependencies(
                        ...
                        addUpdateObserver: { [unowned self] observationID, session, observer in
                            self.sessionRuntimeManager.addUpdateObserver(
                                id: observationID, for: session, observer: observer)
                        },
                        ...
                        isRemoteController: { [unowned self] sessionID, pairedDeviceID in
                            self.sessionControllerRegistry.isRemoteController(
                        ...
                        stabilizedScreenAfterTerminalInput: { [unowned self] in
                            self.stabilizedScreenAfterTerminalInput(
                                for: $0, screenBeforeInput: $1, immediateResponseScreen: $2)
                        },
        ...
        func getHostDetail(hostID: UUID) throws -> NexusDomain.HostDetail {
            ...
            return NexusDomain.HostDetail(
                host: host, latestValidation: try metadataStore.hostValidation(hostID: hostID))
        }
        ...
        func searchNavigation(query: String) throws -> [NavigationItem] {
            ...
            let providerItems = try workspaces.flatMap { workspace in
                try ProviderID.allCases.compactMap { providerID -> NavigationItem? in
                    ...
                    return try providerMatchesQuery(
                        tokens: tokens, normalizedQuery: normalizedQuery, providerID: providerID, workspace: workspace)
                        ? item : nil
                }
            }
            ...
            let sessionItems = try sessionRecordStore.listAllSessions().compactMap { session -> NavigationItem? in
                ...
                return try sessionMatchesQuery(
                    tokens: tokens, normalizedQuery: normalizedQuery, session: session, workspace: workspace)
                    ? item : nil
            }
        ...
        func getProviderDetail(workspaceID: UUID, providerID: ProviderID) throws -> ProviderDetail {
            try AsyncOperationSupport.blocking {
                try await self.getProviderDetail(workspaceID: workspaceID, providerID: providerID)
            }
        }
        ...
        func launchOrResumeDefaultSession(workspaceID: UUID, providerID: ProviderID) throws -> Session {
            try AsyncOperationSupport.blocking {
                try await self.launchOrResumeDefaultSession(workspaceID: workspaceID, providerID: providerID)
            }
        }
        ...
        func createNamedSession(workspaceID: UUID, providerID: ProviderID, name: String?) throws -> Session {
            try AsyncOperationSupport.blocking {
                try await self.createNamedSession(workspaceID: workspaceID, providerID: providerID, name: name)
            }
        }
        ...
        private func resetPiSession(_ session: Session) throws -> SessionScreen {
            ...
            let readySession = try sessionRecordStore.updateSession(
                id: resolvedSession.id, state: .ready, failureMessage: nil)
        ...
        func getStructuredSessionHistoryPage(
            ...
            return StructuredSessionHistoryPage(
                ...
                providerEvents: PiStructuredSessionProviderEventCompaction.compacted(
                    events: page.providerEvents, providerID: .pi),
        ...
        func getStructuredSessionArtifactFile(
            ...
            if let requestingPairedDeviceID {
                guard
                    sessionControllerRegistry.isRemoteController(
                        sessionID: sessionID,
        ...
        func resizeSession(sessionID: UUID, columns: Int, rows: Int) throws -> SessionScreen {
            try AsyncOperationSupport.blocking {
                try await self.resizeSession(sessionID: sessionID, columns: columns, rows: rows)
            }
        }
        ...
        func takeRemoteSessionControl(sessionID: UUID, pairedDeviceID: UUID, columns: Int, rows: Int) throws
            -> SessionScreen
        {
            try AsyncOperationSupport.blocking {
        ...
        func sendRemoteSessionInput(sessionID: UUID, pairedDeviceID: UUID, prompt: SessionPrompt) throws
            -> SessionScreen
        {
            try AsyncOperationSupport.blocking {
                try await self.sendRemoteSessionInput(
                    sessionID: sessionID, pairedDeviceID: pairedDeviceID, prompt: prompt)
            }
        }
        ...
        func sendRemoteSessionInputKey(sessionID: UUID, pairedDeviceID: UUID, key: SessionInputKey) throws
            -> SessionScreen
        {
            try AsyncOperationSupport.blocking {
        ...
        func releaseRemoteSessionControl(sessionID: UUID, pairedDeviceID: UUID) throws -> SessionScreen {
            ...
            guard
                let macSize = sessionControllerRegistry.releaseRemoteControl(
                    sessionID: resolvedSession.id, pairedDeviceID: pairedDeviceID)
            else {
                throw NexusSessionControlError.remoteControllerRequired
            }

            let resizedScreen = try sessionRuntimeManager.resize(
                session: resolvedSession, columns: macSize.columns, rows: macSize.rows)
            try sessionRecordStore.updateSessionTerminalSize(
                id: resolvedSession.id,
                columns: resizedScreen.terminalColumns,
                rows: resizedScreen.terminalRows
        ...
        private func claimMacController(for session: Session) throws -> SessionScreen {
            ...
            guard let macSize = sessionControllerRegistry.claimMacControl(sessionID: session.id),
                currentScreen.terminalColumns != macSize.columns || currentScreen.terminalRows != macSize.rows
            else {
                return currentScreen
            }

            let resizedScreen = try sessionRuntimeManager.resize(
                session: session, columns: macSize.columns, rows: macSize.rows)
            try sessionRecordStore.updateSessionTerminalSize(
                id: session.id,
                columns: resizedScreen.terminalColumns,
                rows: resizedScreen.terminalRows
        ...
        private func stabilizedScreenAfterTerminalInput(
            ...
            while DispatchTime.now().uptimeNanoseconds < deadline {
                ...
                guard
                    let refreshedScreen = try? normalizedSessionScreen(
                        sessionRuntimeManager.sessionScreen(for: session))
                else {
                    break
                }
            ...
            let transitionPlan = try await providerModule.planSessionTransition(
                .bootstrapReadyWithoutRuntime(
                    ProviderModuleReadyWithoutRuntimeBootstrapRequest(
                        session: resolvedSession,
                        workspace: workspace,
                        persistedPrimarySurface: try persistedPrimarySurface(
                            for: resolvedSession, workspace: workspace),
                        storedMetadata: try sessionRecordStore.sessionRecordAdapterMetadata(
                            sessionID: resolvedSession.id)
                    )
                )
            )
            guard case .bootstrapReadyWithoutRuntime(let plan) = transitionPlan else {
        ...
        private func executePersistedSessionLaunch(_ execution: PersistedSessionLaunchExecution) async throws -> Session
        {
            ...
            guard case .relaunchPersisted(let plan) = transitionPlan else {
            ...
            case .recoverRemoteRuntime(let freshRemoteRelaunch):
                return try await recoverRemotePersistedSession(
                    execution,
                    freshRemoteRelaunch: freshRemoteRelaunch,
                    providerModule: providerModule
                )
            case .launchFreshRemoteRuntime(let freshRemoteRelaunch):
        ...
        private func recoverRemotePersistedSession(
            ...
            let readySession =
                execution.session.state == .ready && execution.session.failureMessage == nil
                ? execution.session
        ...
        private func recoverRemoteSession(
            ...
            let readySession =
                session.state == .ready && session.failureMessage == nil
                ? session
        ...
        private func persistRemoteRuntimeRecoveryFailure(
            ...
            let failure = providerModuleRegistry.module(for: session.providerID).remoteRuntimeRecoveryFailure(
                for: failureContext)
        ...
        private func handlePiSessionTransitionAfterRuntimeChange(sessionID: UUID) {
            guard let sourceSession = (try? sessionRecordStore.session(id: sessionID)) ?? nil,
                ...
                let targetLinkage = transition.sessionRecordAdapterMetadata.piSessionLinkage
            else {
                return
            }
        ...
        private func applyPiSessionTransition(from sourceSession: Session, targetLinkage: PiSessionLinkage) throws {
            let existingSessions = try sessionRecordStore.listSessions(
                workspaceID: sourceSession.workspaceID, providerID: sourceSession.providerID)
            ...
            let targetSession =
                if let existingTargetSession {
                    try sessionRecordStore.updateSession(
                        id: existingTargetSession.id, state: .ready, failureMessage: nil)
                } else {
        ...
        private func matchingPiSessionRecord(
            ...
            for session in sessions where session.id != sourceSessionID {
                guard
                    let linkage = try sessionRecordStore.sessionRecordAdapterMetadata(sessionID: session.id)?
                        .piSessionLinkage
                else {
                    continue
                }
        ...
        private func createPiTransitionNamedSession(from sourceSession: Session, existingSessions: [Session]) throws
            -> Session
        {
            ...
            try sessionRecordStore.updateSessionTerminalSize(
                id: session.id, columns: terminalSize.columns, rows: terminalSize.rows)
            return session
        }
        ...
        private func persistPrimarySurfaceIfNeeded(for session: Session, primarySurface: SessionSurface) throws {
            guard let launchSnapshot = try sessionRecordStore.launchSnapshot(sessionID: session.id),
                launchSnapshot.primarySurface != primarySurface
            else {
                return
            }

            try sessionRecordStore.updateLaunchSnapshotPrimarySurface(
                sessionID: session.id, primarySurface: primarySurface)
        }
        ...
        private func runtimeLaunchConfiguration(
            ...
            if workspace.kind == .remote {
                ...
                remoteHost = host
                resolvedRemoteRuntimeIdentifier = try remoteRuntimeIdentifier(
                    for: session, forceNew: forceFreshRemoteRuntime)
            } else {
                remoteHost = nil
        ...
        private func persistedPiStructuredSessionState(for session: Session) throws
            -> PiStructuredSessionPersistedState?
        {
        ...
        private func persistedApprovalRequests(for session: Session) throws -> [SessionApprovalRequest] {
            ...
            return try sessionRecordStore.sessionRecordAdapterMetadata(sessionID: session.id)?
                .piPersistedApprovalRequests ?? []
        }
        ...
        private func persistedProviderEvents(for session: Session) throws -> [SessionProviderEvent] {
            ...
            if let state = try persistedPiStructuredSessionState(for: session) {
                ...
                providerEvents =
                    ...
                    .piPersistedProviderEvents ?? []
            }

            return PiStructuredSessionProviderEventCompaction.compacted(
                events: providerEvents, providerID: session.providerID)
        }
        ...
        private func staticSessionTranscript(
            ...
            guard primarySurface == .structuredActivityFeed,
                ...
                persistedActivityItems.isEmpty == false
            else {
        ...
        private func navigationItem(_ target: NavigationTarget) throws -> NavigationItem? {
            ...
            case .workspace:
                guard let workspaceID = target.workspaceID,
                    let workspace = try metadataStore.workspace(id: workspaceID)
                else {
                    return nil
                }
        ...
        private func providerMatchesQuery(
            tokens: [String], normalizedQuery: String, providerID: ProviderID, workspace: Workspace
        ) throws -> Bool {
        ...
        private func sessionMatchesQuery(
            tokens: [String], normalizedQuery: String, session: Session, workspace: Workspace
        ) throws -> Bool {
            ...
            return try itemMatchesAllTokens(
                tokens, fields: [sessionName, providerName] + workspaceSearchFields(workspace))
        }
        ...
        fileprivate static func renderTerminalState(
            ...
            func insertBlankLinesWithinRegion(_ count: Int, at lineIndex: Int) {
                ...
                let replacement =
                    Array(repeating: [Character](), count: insertCount) + Array(regionSlice.dropLast(insertCount))
            ...
            func deleteLinesWithinRegion(_ count: Int, at lineIndex: Int) {
                ...
                let regionSlice = Array(lines[lineIndex...region.upperBound])
                let replacement =
                    Array(regionSlice.dropFirst(deleteCount)) + Array(repeating: [Character](), count: deleteCount)
                lines.replaceSubrange(lineIndex...region.upperBound, with: replacement)
            }
            ...
            for (lineIndex, line) in sourceLines.enumerated() {
                ...
                if line.isEmpty {
                    ...
                    while startIndex < line.endIndex {
                        let endIndex =
                            line.index(startIndex, offsetBy: columns, limitedBy: line.endIndex) ?? line.endIndex
        ...
        private func resolveNamedSessionName(_ requestedName: String?, existingSessions: [Session]) -> String {
            if let trimmedName = requestedName?.trimmingCharacters(in: .whitespacesAndNewlines),
        ...
        private func readySessionRecordMayBeDeleted(_ session: Session) throws -> Bool {
            guard session.state == .ready,
                let workspace = try metadataStore.workspace(id: session.workspaceID),
                try stopRequiresActiveIBMBobTurn(session, workspace: workspace)
            else {
                return false
            }

            if let runtimeLinkage = sessionRuntimeManager.sessionRecordAdapterMetadata(for: session)?
                .ibmBobSessionLinkage
            {
                return runtimeLinkage.turnInProgress == false
            }

            return try sessionRecordStore.sessionRecordAdapterMetadata(sessionID: session.id)?.ibmBobTurnInProgress
                != true
        }
    ...
    extension NexusService: NSXPCListenerDelegate {
        public func listener(_ listener: NSXPCListener, shouldAcceptNewConnection newConnection: NSXPCConnection)
            -> Bool
        {
    ...
    private final class NexusXPCBridge: NSObject, NexusXPCProtocol, @unchecked Sendable {
        ...
        func createHost(name: String, sshTarget: String, port: NSNumber?, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { try service.createHost(name: name, sshTarget: sshTarget, port: port?.intValue) }, reply: reply)
        }

        func updateHost(
            hostID: String, name: String, sshTarget: String, port: NSNumber?,
            reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: {
                    try service.updateHost(
                        hostID: resolveUUID(hostID), name: name, sshTarget: sshTarget, port: port?.intValue)
                }, reply: reply)
        }
        ...
        func completePairing(pairingCode: String, deviceName: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { try service.completePairing(pairingCode: pairingCode, deviceName: deviceName) }, reply: reply)
        }
        ...
        func getWorkspaceOverview(workspaceID: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in
                    try await self.service.getWorkspaceOverview(workspaceID: self.resolveUUID(workspaceID))
                }, reply: reply)
        }
        ...
        func refreshWorkspaceOverview(workspaceID: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in
                    try await self.service.refreshWorkspaceOverview(workspaceID: self.resolveUUID(workspaceID))
                }, reply: reply)
        }
        ...
        func createLocalWorkspace(
            name: String?, folderPath: String, primaryGroupID: String?, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: {
        ...
        func createRemoteWorkspace(
            name: String?, hostID: String, remotePath: String, primaryGroupID: String?,
            reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: {
        ...
        func launchOrResumeDefaultSession(
            workspaceID: String, providerID: String, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
                    guard let resolvedProviderID = ProviderID(rawValue: providerID) else {
        ...
        func launchOrResumeSession(sessionID: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in try await self.service.launchOrResumeSession(sessionID: self.resolveUUID(sessionID))
                }, reply: reply)
        }

        func createNamedSession(
            workspaceID: String, providerID: String, name: String?, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
                    guard let resolvedProviderID = ProviderID(rawValue: providerID) else {
        ...
        func getSessionScreenObservationSnapshot(sessionID: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { try service.getSessionScreenObservationSnapshot(sessionID: resolveUUID(sessionID)) },
                reply: reply)
        }
        ...
        func observeSessionScreen(sessionID: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: {
                    ...
                    let screenObserver = SessionScreenObserverProxy(
                        connection: connection, observationID: observationID)
                    let start = try service.observeSessionScreen(
                        observationID: observationID, sessionID: resolveUUID(sessionID)
                    ) { update in
        ...
        func sendSessionInput(sessionID: String, text: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in
                    try await self.service.sendSessionInput(sessionID: self.resolveUUID(sessionID), text: text)
                },
                reply: reply
            )
        }
        ...
        func sendSessionPrompt(sessionID: String, promptPayload: Data, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in
                    ...
                    return try await self.service.sendSessionInput(
                        sessionID: self.resolveUUID(sessionID), prompt: prompt)
                },
                reply: reply
            )
        }
        ...
        func sendSessionText(sessionID: String, text: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in
                    try await self.service.sendSessionText(sessionID: self.resolveUUID(sessionID), text: text)
                },
                reply: reply
            )
        }
        ...
        func sendSessionInputKey(sessionID: String, key: String, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in
                    ...
                    return try await self.service.sendSessionInputKey(
                        sessionID: self.resolveUUID(sessionID), key: resolvedKey)
                },
                reply: reply
            )
        }
        ...
        func respondToApprovalRequest(
            sessionID: String, approvalRequestID: String, decision: String, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
        ...
        func respondToExtensionDialog(
            sessionID: String, dialogID: String, responsePayload: Data, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
                    let response = try JSONDecoder().decode(
                        SessionExtensionUIDialogResponse.self, from: responsePayload)
        ...
        func resizeSession(sessionID: String, columns: Int, rows: Int, reply: @escaping (Data?, NSString?) -> Void) {
            sendReply(
                with: { [self] in
                    try await self.service.resizeSession(
                        sessionID: self.resolveUUID(sessionID), columns: columns, rows: rows)
                },
                reply: reply
            )
        }

        func takeRemoteSessionControl(
            sessionID: String, pairedDeviceID: String, columns: Int, rows: Int,
            reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
        ...
        func releaseRemoteSessionControl(
            sessionID: String, pairedDeviceID: String, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: {
        ...
        func sendRemoteSessionInput(
            sessionID: String, pairedDeviceID: String, text: String, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
        ...
        func sendRemoteSessionPrompt(
            sessionID: String, pairedDeviceID: String, promptPayload: Data, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
                    let prompt = try JSONDecoder().decode(SessionPrompt.self, from: promptPayload)
        ...
        func respondToRemoteApprovalRequest(
            sessionID: String, pairedDeviceID: String, approvalRequestID: String, decision: String,
            reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
        ...
        func respondToRemoteExtensionDialog(
            sessionID: String, pairedDeviceID: String, dialogID: String, responsePayload: Data,
            reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
        ...
        func sendRemoteSessionInputKey(
            sessionID: String, pairedDeviceID: String, key: String, reply: @escaping (Data?, NSString?) -> Void
        ) {
            sendReply(
                with: { [self] in
                    guard let resolvedKey = SessionInputKey(rawValue: key) else {
        ...
        private func sendReply<T: Encodable>(
            with operation: () throws -> T, reply: @escaping (Data?, NSString?) -> Void
        ) {
            do {
        ...
        private func sendReply<T: Encodable>(
            with operation: @escaping @Sendable () async throws -> T, reply: @escaping (Data?, NSString?) -> Void
        ) {
    ...
    private final class SessionScreenObserverProxy: @unchecked Sendable {
        ...
        func send(_ update: SessionScreenObservationUpdate) {
            ...
            guard
                let observer = connection.remoteObjectProxyWithErrorHandler({ _ in })
                    as? NexusSessionScreenObserverXPCProtocol
            else {
                return
            }
    ...
    public enum NexusEmbeddedServiceBootstrap {
        public static func bootstrap() throws -> any NexusEmbeddedServiceSession {
            throw NSError(
                domain: "NexusService", code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Nexus embedded services are only available on macOS"])
        }
