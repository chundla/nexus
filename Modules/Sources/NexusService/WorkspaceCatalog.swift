    ...
    final class WorkspaceCatalog: WorkspaceCatalogReading, @unchecked Sendable {
            ...
            do {
                guard
                    let workspace = try trace.measure(
                        "loadWorkspace", { try dependencies.metadataStore.workspace(id: workspaceID) })
                else {
        ...
        private func providerDetailRemoteContext(
            ...
            guard
                providerModule(for: providerID).supportsSharedRemoteProbeFacts(
                    with: dependencies.providerHealthEvaluator)
            else {
                return try remoteWorkspaceHealthContext(for: workspace)
            }
        ...
        private func storedRemoteWorkspaceHealthContext(for workspace: Workspace) throws
            -> RemoteWorkspaceHealthContext?
        {
            guard workspace.kind == .remote,
                let hostID = workspace.remoteHostID,
                let host = try dependencies.metadataStore.host(id: hostID),
                let workspaceAvailability = try dependencies.metadataStore.workspaceAvailability(
        ...
        func remoteWorkspaceTargetOverview(
            ...
            if refreshHostValidation == false,
                let existingAvailability = try dependencies.metadataStore.workspaceAvailability(
                    workspaceID: workspace.id),
        ...
        private func workspaceOverviewRemoteTargetOverview(
            ...
            case .forceFresh:
                let probeFactsCollection = dependencies.remoteWorkspaceProbeCollector.collect(
        ...
        private func workspaceProviderCards(
            ...
            let loadedProviderCards = try await withThrowingTaskGroup(
                of: LoadedWorkspaceProviderCard.self, returning: [LoadedWorkspaceProviderCard].self
            ) { group in
                ...
                var orderedProviderCards = [LoadedWorkspaceProviderCard?](repeating: nil, count: providerIDs.count)
        ...
        private func loadWorkspaceProviderCard(
            ...
            let defaultSession = sessions.first(where: \.isDefault)
            let (catalogReadResult, readProviderCatalogStep) = try await measuredStep(
                "readProviderCatalog.\(providerID.rawValue)"
            ) {
        ...
        private func workspaceOverviewProviderHealthSummary(
            ...
            switch mode {
            case .browse:
                if let snapshot = try dependencies.metadataStore.providerHealth(
                    workspaceID: workspace.id, providerID: providerID)
                {
        ...
        private func hostValidationResult(
            ...
            switch probeFactsCollection {
            case .transportFailed(let detail), .rawProbeFailed(let detail):
        ...
        private func workspaceAvailabilityResult(
            ...
            guard case .collected(let facts) = probeFactsCollection else {
                return WorkspaceAvailabilityResult(
                    state: .blocked,
                    summary: "Workspace Availability is blocked by Host Validation",
                    diagnostics: [
        ...
        private func measuredStep<T>(_ name: String, _ block: () async throws -> T) async rethrows -> (
            T, PerformanceDiagnosticStep
        ) {
            let startedAt = dependencies.currentUptimeNanoseconds()
