    ...
    protocol CodexProviderHealthFactProviding: Sendable {
        ...
        func probeRemoteCodexReadiness(workspace: Workspace, host: NexusDomain.Host, executable: String) async
            -> RemoteCodexReadinessProbeResult
    }
    ...
    protocol PiProviderHealthFactProviding: Sendable {
        ...
        func probeRemotePiReadiness(workspace: Workspace, host: NexusDomain.Host, executable: String) async
    ...
    extension ProviderHealthEvaluating {
        func healthSummary(
            ...
            (try? AsyncOperationSupport.blocking {
                await healthSummary(for: providerID, workspace: workspace, remoteContext: remoteContext)
            })
                ?? ProviderHealthSummary(
                    state: .notChecked,
        ...
        SharedRemoteIBMBobProviderHealthFactProviding, @unchecked Sendable
    {
        ...
        func healthSummary(
            for providerID: ProviderID, workspace: Workspace, remoteContext: RemoteWorkspaceHealthContext? = nil
        ) async -> ProviderHealthSummary {
        ...
        func remoteCLIHealthProbe(
            ...
            do {
                let result = try commandRunner.run(
                    ...
                    arguments: remoteCLIHealthProbeArguments(
                        commandName: commandName, workspace: workspace, host: host),
                ...
                return .ready(
                    ...
                    diagnostics: [
                        ProviderHealthDiagnostic(
                            ...
                            code: "remoteProbe",
                            message:
                                "Validated remote \(providerName) launch prerequisites on \(host.name) for \(workspace.folderPath)."
                        )
                    ]
                )
            } catch {
        ...
        func probeRemoteCodexReadiness(
            ...
            do {
                switch try await remoteCodexReadinessProbe.probe(
                    host: host, executable: executable, workingDirectory: workspace.folderPath)
                {
                case .ready:
                    return .ready
        ...
        func probeRemotePiReadiness(
            ...
            do {
                switch try await remotePiReadinessProbe.probe(
                    host: host, executable: executable, workingDirectory: workspace.folderPath)
                {
                case .ready:
                    return .ready
        ...
        func remoteIBMBobPassiveProbe(
            workspace: Workspace, host: NexusDomain.Host, probeFacts: RemoteWorkspaceProbeFacts
        ) async -> RemoteIBMBobPassiveProbeResult {
            ...
            -> RemoteIBMBobPassiveProbeResult
        {
            ...
            do {
                readinessProbe = try commandRunner.run(
                    executable: "/usr/bin/ssh",
                    arguments: remoteIBMBobPassiveProbeArguments(
                        executable: executable, workspace: workspace, host: host),
                    currentDirectoryURL: nil
                )
            } catch {
        ...
        func localCodexExecutableFacts(workspace: Workspace) async -> LocalCodexExecutableFacts {
            ...
            let version =
                resolution.resolvedExecutable.map {
            ...
            do {
                ...
                return .passiveProbeCompleted(
                    ...
                    detail: launchProbe.exitStatus == 0
                        ? nil
                        : launchProbeFailureMessage(
                            stdout: launchProbe.stdout, stderr: launchProbe.stderr, providerName: "IBM Bob")
                )
            } catch {
            ...
            -> LocalCLIHealthProbeResult
        {
            ...
            var diagnostics: [ProviderHealthDiagnostic] = []
            let version = detectLocalVersion(
                executable: executable, providerName: providerName, diagnostics: &diagnostics)
        ...
        private func runLocalCommandThroughShell(
            ...
            throw lastError
                ?? NSError(
                    ...
                    code: 1,
                    userInfo: [
                        NSLocalizedDescriptionKey: "Local shell launch probe failed before the command completed."
                    ]
                )
        }

        private func detectLocalVersion(
            executable: String, providerName: String, diagnostics: inout [ProviderHealthDiagnostic]
        ) -> String? {
            do {
                let result = try runLocalCommandThroughShell(
                    executable: executable, arguments: ["--version"], currentDirectoryURL: nil)
                guard result.exitStatus == 0 else {
                    diagnostics.append(
                        ProviderHealthDiagnostic(
                            severity: .warning,
                            code: "versionUnavailable",
                            message: launchProbeFailureMessage(
                                stdout: result.stdout, stderr: result.stderr, providerName: providerName)
                        )
                    )
                    return nil
                }
        ...
        private func remoteIBMBobPassiveProbeArguments(executable: String, workspace: Workspace, host: NexusDomain.Host)
            -> [String]
        {
            var arguments = [
                "-o", "BatchMode=yes",
        ...
        private func remoteCLIHealthProbeArguments(commandName: String, workspace: Workspace, host: NexusDomain.Host)
            -> [String]
        {
            var arguments = [
                "-o", "BatchMode=yes",
    ...
    struct CodexAppServerReadinessProbe: CodexReadinessProbing {
            ...
            transport.setStdoutLineHandler { line in
                    ...
                    let message = error["message"] as? String
                {
                    let resolvedError = NSError(
                        domain: "CodexAppServerReadinessProbe", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
                ...
                let resolvedError = NSError(
                    ...
                    userInfo: [
                        NSLocalizedDescriptionKey: detail?.isEmpty == false
                            ? detail! : "Codex app-server exited before readiness completed."
                    ]
                )
                state.record(error: resolvedError)
                waiter.fail(resolvedError)
                        ...
                        NSError(
                            ...
                            userInfo: [
                                NSLocalizedDescriptionKey:
                                    "Codex app-server did not answer the readiness probe in time."
                            ]
                        )
                    }
                )
        ...
        private static func jsonLine(_ object: [String: Any]) throws -> String {
            ...
            guard let line = String(data: data, encoding: .utf8) else {
                throw NSError(
                    domain: "CodexAppServerReadinessProbe", code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "Failed to encode Codex readiness probe request."])
            }
            return line
        }
    }
    ...
    struct SSHRemoteCodexAppServerReadinessProbe: RemoteCodexReadinessProbing, @unchecked Sendable {
        typealias TransportFactory =
            @Sendable (_ executable: String, _ arguments: [String], _ workingDirectory: String?) throws ->
            ...
            -> RemoteCodexReadinessOutcome
        {
            ...
            transport.setStdoutLineHandler { line in
                ...
                {
                    let resolvedError = NSError(
                        domain: "SSHRemoteCodexAppServerReadinessProbe", code: 1,
            ...
            transport.setTerminationHandler { termination in
                ...
                let resolvedError = NSError(
                    ...
                    userInfo: [
                        NSLocalizedDescriptionKey: detail?.isEmpty == false
                            ? detail! : "Codex app-server exited before remote readiness completed."
                    ]
                )
                state.record(error: resolvedError)
                waiter.fail(resolvedError)
                ...
                try await waiter.wait(
                    ...
                    timeoutError: {
                        NSError(
                            ...
                            userInfo: [
                                NSLocalizedDescriptionKey:
                                    "Codex app-server did not answer the remote readiness probe in time."
                            ]
                        )
                    }
                )
        ...
        private static func jsonLine(_ object: [String: Any]) throws -> String {
            ...
            guard let line = String(data: data, encoding: .utf8) else {
                throw NSError(
                    domain: "SSHRemoteCodexAppServerReadinessProbe", code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "Failed to encode remote Codex readiness probe request."])
            }
            return line
        }
    }
    ...
    struct SSHRemotePiRPCReadinessProbe: RemotePiReadinessProbing, @unchecked Sendable {
            ...
            -> RemotePiReadinessOutcome
        {
            ...
            transport.setStdoutLineHandler { line in
                guard let data = line.data(using: .utf8),
                    ...
                    object["id"] as? String == responseID
                else {
                    return
                }
                ...
                let resolvedError = NSError(
                    domain: "SSHRemotePiRPCReadinessProbe", code: 1, userInfo: [NSLocalizedDescriptionKey: message])
                state.record(error: resolvedError)
                waiter.fail(resolvedError)
            }
                ...
                try await waiter.wait(
                    ...
                    timeoutError: {
                        NSError(
                            ...
                            userInfo: [
                                NSLocalizedDescriptionKey:
                                    "Pi RPC mode did not answer the remote readiness probe in time."
                            ]
                        )
                    }
                )
        ...
        private static func jsonLine(_ object: [String: Any]) throws -> String {
            ...
            guard let line = String(data: data, encoding: .utf8) else {
                throw NSError(
                    domain: "SSHRemotePiRPCReadinessProbe", code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "Failed to encode remote Pi readiness probe request."])
            }
            return line
        }
    }
