    ...
    final class NexusMetadataStore {
        ...
        init(storeURL: URL) throws {
            ...
            guard result == SQLITE_OK, let database else {
                let message =
                    database.flatMap { String(cString: sqlite3_errmsg($0)) } ?? "Could not open metadata store"
            ...
            try withLock {
                ...
                let statement = try prepare(
                    ...
                    INSERT INTO performance_diagnostics (
                        id, operation, outcome, workspace_id, provider_id, session_id, total_elapsed_ms, steps_json, metrics_json, failure_message, recorded_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
                    """
                )
                defer { sqlite3_finalize(statement) }
                ...
                try bind(try encodePerformanceDiagnosticMetrics(diagnostic.metrics), at: 9, in: statement)
                try bind(diagnostic.failureMessage, at: 10, in: statement)
                try bind(recordedAtMilliseconds, at: 11, in: statement)
                try stepDone(statement)
        ...
        func listPerformanceDiagnostics(limit: Int) throws -> [PerformanceDiagnosticRecord] {
            ...
            return try withLock {
                let statement = try prepare(
                    """
                    SELECT id, operation, outcome, workspace_id, provider_id, session_id, total_elapsed_ms, steps_json, metrics_json, failure_message, recorded_at
        ...
        func saveHostValidation(hostID: UUID, result: HostValidationResult, checkedAt: Date) throws
            -> HostValidationSnapshot
        {
            try withLock {
        ...
        func saveWorkspaceAvailability(workspaceID: UUID, result: WorkspaceAvailabilityResult, checkedAt: Date) throws
        ...
        func saveProviderHealth(
            workspaceID: UUID, providerID: ProviderID, summary: ProviderHealthSummary, checkedAt: Date
        ...
        private func saveSessionRecordAdapterMetadataWithoutLock(
            sessionID: UUID, metadata: SessionRecordAdapterMetadata
        ) throws {
            let statement = try prepare(
                """
        ...
        private func readRemoteClientDiagnosticBreadcrumb(from statement: OpaquePointer?) throws
        ...
        private func readPerformanceDiagnostic(from statement: OpaquePointer?) throws -> PerformanceDiagnosticRecord {
            ...
            return PerformanceDiagnosticRecord(
                ...
                metrics: try decodePerformanceDiagnosticMetrics(from: readString(column: 8, from: statement)),
                failureMessage: readOptionalString(column: 9, from: statement),
                recordedAt: Date(timeIntervalSince1970: Double(sqlite3_column_int64(statement, 10)) / 1_000)
            )
        }
        ...
        private func readHostValidationSnapshot(from statement: OpaquePointer?) throws -> HostValidationSnapshot {
            ...
            return HostValidationSnapshot(
                hostID: hostID, state: state, summary: summary, checkedAt: checkedAt, diagnostics: diagnostics)
        }

        private func readWorkspaceAvailabilitySnapshot(from statement: OpaquePointer?) throws
            -> WorkspaceAvailabilitySnapshot
        {
            ...
            return WorkspaceAvailabilitySnapshot(
                workspaceID: workspaceID, state: state, summary: summary, checkedAt: checkedAt, diagnostics: diagnostics
            )
        }
        ...
        private func encodeWorkspaceAvailabilityDiagnostics(_ diagnostics: [WorkspaceAvailabilityDiagnostic]) throws
            -> String
        {
            let data = try JSONEncoder().encode(diagnostics)
            guard let json = String(data: data, encoding: .utf8) else {
        ...
        private func decodeWorkspaceAvailabilityDiagnostics(from json: String) throws
        ...
        private func encodePerformanceDiagnosticMetrics(_ metrics: [String: Int]) throws -> String {
            let data = try JSONEncoder().encode(metrics)
            guard let json = String(data: data, encoding: .utf8) else {
                throw NexusMetadataStoreError.sqlite("Could not encode performance diagnostic metrics")
            }
            return json
        }

        private func decodePerformanceDiagnosticMetrics(from json: String) throws -> [String: Int] {
            guard let data = json.data(using: .utf8) else {
                throw NexusMetadataStoreError.sqlite("Could not decode performance diagnostic metrics")
            }
