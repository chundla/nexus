    ...
    final class ServiceSessionLifecycle: SessionLifecycleManaging {
        ...
        func launchOrResumeSession(sessionID: UUID) async throws -> Session {
            ...
            do {
                guard
                    let session = try trace.measure(
                        "loadSession", { try dependencies.sessionRecordStore.session(id: sessionID) })
                else {
        ...
        func launchOrResumeDefaultSession(workspaceID: UUID, providerID: ProviderID) async throws -> Session {
            ...
            do {
                ...
                if let existingSession = try trace.measure(
                    "loadDefaultSession",
                    {
                        try dependencies.sessionRecordStore.defaultSession(
                            workspaceID: workspaceID, providerID: providerID)
                    })
                {
                    let launchedSession = try await launchPersistedSession(
                        existingSession, workspace: workspace, trace: &trace)
                    try? dependencies.recordPerformanceDiagnostic(trace.finish(outcome: .success))
                    return launchedSession
                }
        ...
        private func openFreshSession(
            ...
            guard case .openFresh(let openResult) = transitionPlan else {
        ...
        private func launchPersistedSession(
            ...
            if let launchSnapshot = try trace.measure(
                "loadLaunchSnapshot",
                {
                ...
                })
            {
                ...
                let mode: PersistedSessionLaunchMode =
                    if shouldAttemptRemoteRuntimeRecovery(for: reconciledSession, workspace: workspace) {
                        ...
                        .launch(
                            forceFreshRemoteRuntime: shouldCreateFreshRemoteRuntime(
                                for: reconciledSession, workspace: workspace))
                    }
        ...
        private func relaunchSessionRecordAdapterMetadataSource(for session: Session) throws
