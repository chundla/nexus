    ...
    enum IBMBobSessionRuntimeError: LocalizedError, Equatable {
        ...
        var errorDescription: String? {
            ...
            case .busy:
                return
                    "IBM Bob is already handling a prompt. Wait for the current turn to finish before sending another one."
            case .noActiveTurnToStop:
    ...
    final class IBMBobSessionRuntime: SessionRuntime, @unchecked Sendable {
        ...
        init(
            ...
            let restoredActivityItems = StructuredSessionLiveHistoryRetention.retainedActivityItems(
                sessionLinkage?.persistedActivityItems ?? [])
            self.activityItems = restoredActivityItems.isEmpty ? Self.defaultActivityItems : restoredActivityItems
            self.transcriptEntries = StructuredSessionLiveHistoryRetention.retainedTranscriptEntries(
                Self.transcriptEntries(from: self.activityItems))
        }
        ...
        private func bobEvent(from object: [String: Any]) -> BobEvent? {
            ...
            case "tool_result":
                ...
                let text =
                    stringValue(in: object, keys: ["output", "text", "error", "message", "detail"])
                    ?? "IBM Bob reported an error."
                return BobEvent(kind: .error, text: text)
            case "diff", "patch":
                guard let text = stringValue(in: object, keys: ["text", "diff", "patch", "message"]),
                    text.isEmpty == false
                else {
                    return nil
                }
            ...
            case "error", "tool_result_error":
                let text =
                    stringValue(in: object, keys: ["text", "error", "message", "detail"])
                    ?? "IBM Bob reported an error."
                return BobEvent(kind: .error, text: text)
            default:
                return nil
        ...
        private func resolvedSessionID(from object: [String: Any]) -> String? {
            if let sessionID = stringValue(
                in: object, keys: ["session_id", "sessionId", "conversation_id", "conversationId"])
            {
                return sessionID
            }
        ...
        private static func discoverSlashCommands(executable: String, workingDirectory: String)
            -> [SessionSlashCommand]?
        {
            ...
            for command in loadBobCustomCommands(
                from: homeDirectory.appendingPathComponent(".bob/commands", isDirectory: true), location: .user)
            {
                commandsByName[command.name] = command
            }
            for command in loadBobCustomModes(
                from: homeDirectory.appendingPathComponent(".bob/custom_modes.yaml", isDirectory: false),
                location: .user)
            {
                commandsByName[command.name] = command
            }
            for command in loadBobCustomCommands(
                from: projectRoot.appendingPathComponent(".bob/commands", isDirectory: true), location: .project)
            {
                commandsByName[command.name] = command
            }
            for command in loadBobCustomModes(
                from: projectRoot.appendingPathComponent(".bob/custom_modes.yaml", isDirectory: false),
                location: .project)
            {
                commandsByName[command.name] = command
            }

            let commands = commandsByName.values.sorted {
        ...
        private static func loadBobCustomCommands(from directory: URL, location: SessionSlashCommandLocation)
            -> [SessionSlashCommand]
        {
            let fileManager = FileManager.default
            ...
            let urls = (fileManager.enumerator(at: directory, includingPropertiesForKeys: nil)?.allObjects ?? [])
                .compactMap { $0 as? URL }
            ...
            return
                ...
                .compactMap { fileURL in
                    ...
                    let commandName = (relativePath as NSString).deletingPathExtension.replacingOccurrences(
                        of: "\\", with: "/")
        ...
        private static func loadBobCustomModes(from fileURL: URL, location: SessionSlashCommandLocation)
            -> [SessionSlashCommand]
        {
            ...
            func finalize(_ mode: PendingMode) -> SessionSlashCommand? {
                guard let slug = mode.slug?.trimmingCharacters(in: .whitespacesAndNewlines), slug.isEmpty == false
                else {
                    return nil
                }
                ...
                let summary =
                    (trimmedWhenToUse?.isEmpty == false ? trimmedWhenToUse : nil)
