    ...
    enum ShellSupport {
        ...
        static func localShellCandidates(
            ...
            for candidate in [environment["SHELL"], loginShell].compactMap({ $0 }) + etcShells(fileManager: fileManager)
                + commonShellPaths
            {
        ...
        private static func etcShells(fileManager: FileManager) -> [String] {
            ...
            guard fileManager.isReadableFile(atPath: shellsPath),
                let contents = try? String(contentsOfFile: shellsPath, encoding: .utf8)
            else {
                return []
            }
