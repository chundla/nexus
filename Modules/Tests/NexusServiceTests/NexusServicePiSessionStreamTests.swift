    ...
    private final class BlockingObserverProbePiRPCTransport: PiRPCTransporting, @unchecked Sendable {
        private var stdoutLineHandler: (@Sendable (String) -> Void)?
        private var terminationHandler: (@Sendable (Int32) -> Void)?
        let firstPromptEventReturned = DispatchSemaphore(value: 0)
        let secondPromptEventReturned = DispatchSemaphore(value: 0)
        ...
        func sendLine(_ line: String) throws {
            ...
            case "prompt":
                ...
                DispatchQueue.global().async {
                    ...
                    self.firstPromptEventReturned.signal()
                    self.emit([
                        "type": "tool_execution_start",
                        "toolCallId": "tool-2",
                        "toolName": "LS",
                    ...
                    self.secondPromptEventReturned.signal()
    ...
    private func waitForSignal(_ semaphore: DispatchSemaphore, timeoutNanoseconds: UInt64 = 300_000_000) async -> Bool {
    ...
    extension NexusServicePiSessionStreamTests {
        @Test func localPiRuntimeChangeNotificationsDoNotBlockPromptEventDrain() async throws {
            let transport = BlockingObserverProbePiRPCTransport()
            ...
            runtime.setChangeHandler {
                guard callbackCount.increment() >= 2 else {
                    return
                }
            ...
            let sendTask = Task.detached {
                try runtime.sendInput("inspect")
            }

            #expect(await waitForSignal(transport.firstPromptEventReturned))
            #expect(await waitForSignal(transport.secondPromptEventReturned))

            unblockObserver.signal()
            _ = try await sendTask.value

            let screen = runtime.sessionScreen(for: session)
            #expect(screen.activityItems.contains { $0.kind == .command && $0.text == "read CONTEXT.md" })
            #expect(screen.activityItems.contains { $0.kind == .command && $0.text == "LS" })
        }
