            ...
            let service = try NexusService.bootstrapForTests(
                ...
                providerHealthEvaluator: ProviderHealthFacts(
                    ...
                    commandRunner: CompatibilityStubCommandRunner(results: [
                        .init(executable: "/bin/zsh", arguments: ["-lic", "'/tmp/fake-claude' '--version'"]): .success(
                            stdout: "9.9.9 (Claude Code)\n"),
                        .init(executable: "/bin/zsh", arguments: ["-lic", "'/tmp/fake-claude' '--help'"]): .success(
                            stdout: "Usage: claude\n"),
    ...
    private struct CompatibilityStubCommandRunner: ProviderCommandRunning {
        ...
        func run(executable: String, arguments: [String], currentDirectoryURL: URL?) throws -> ProviderCommandResult {
            guard let result = results[Invocation(executable: executable, arguments: arguments)] else {
                throw NSError(
                    domain: "CompatibilityStubCommandRunner", code: 1,
