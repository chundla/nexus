    ...
    struct TestProviderModule: ProviderModule {
        ...
        private let healthSummaryEvaluator:
            (Workspace, RemoteWorkspaceHealthContext?, any ProviderHealthEvaluating) async -> ProviderHealthSummary
        ...
        private let remoteRuntimeRecoveryFailureEvaluator:
            (RemoteRuntimeRecoveryFailureContext) -> (state: Session.State, message: String)
        ...
        private let runtimeConstructor:
            (
                (Session, Workspace, SessionRuntimeLaunchConfiguration, ProviderModuleRuntimeConstructionActions)
        ...
        init(
            ...
            healthSummaryEvaluator:
                @escaping (Workspace, RemoteWorkspaceHealthContext?, any ProviderHealthEvaluating) async ->
            ...
            remoteRuntimeRecoveryFailureEvaluator: (
                (RemoteRuntimeRecoveryFailureContext) -> (state: Session.State, message: String)
            )? = nil,
            remoteHealthSnapshotReuseEvaluator:
                @escaping (ProviderHealthSummary, RemoteWorkspaceHealthContext?) -> Bool = { _, _ in false },
            ...
            self.initialTranscriptBuilder =
                initialTranscriptBuilder ?? { workspace, remoteHost, launchMode in
            ...
            self.interruptedSessionFailureMessageBuilder =
                interruptedSessionFailureMessageBuilder ?? { _, _, _ in
