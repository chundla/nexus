    ...
    struct NexusSessionScreenObserverBridgeTests {
        @Test func bridgeSerializesObservationUpdateDeliveryPerObservation() async throws {
            ...
            bridge.registerHandler(
                { update in
                    guard case .screen(let screen) = update,
                        let marker = screen.activityItems.last?.text
                    else {
                        return
                    }
            ...
            let firstPayload = try JSONEncoder().encode(
                SessionScreenObservationUpdate.screen(makeScreen(marker: "first")))
            let secondPayload = try JSONEncoder().encode(
                SessionScreenObservationUpdate.screen(makeScreen(marker: "second")))
