    ...
    struct ServiceSessionInteractionScenariosTests {
        @Test func observingSessionScreenYieldsCurrentScreenBeforeLaterUpdates() async throws {
            ...
            let start = try service.observeSessionScreen(observationID: UUID(), sessionID: session.id) { update in
                guard case .screen(let screen) = update else {
                    return
                }
                Task {
        ...
        @Test func macStructuredSessionInputAndApprovalDecisionsReturnUpdatedSessionScreen() async throws {
            ...
            #expect(
                pendingScreen.activityItems.map(\.text) == [
                    "Pi ready",
                    "You: deploy",
                    "Approval Request: Deploy?",
                ])
        ...
        @Test func remoteControllerGatesStructuredSessionInputAndApprovalDecisions() async throws {
            ...
            do {
                _ = try await service.sendRemoteSessionInput(
                    sessionID: session.id, pairedDeviceID: pairedDeviceID, text: "deploy")
        ...
        @Test func remoteControllerGatesTerminalTextAndInputKey() async throws {
            ...
            do {
                _ = try await service.sendRemoteSessionText(
                    sessionID: session.id, pairedDeviceID: pairedDeviceID, text: "ls")
    ...
    private final class StructuredControllerRuntime: SessionRuntime, @unchecked Sendable {
        ...
        private let lock = NSLock()
        private var approvalRequest = SessionApprovalRequest(
            id: UUID(), title: "Deploy?", text: "Deploy?", state: .pending)
        ...
        func respondToApprovalRequest(_ approvalRequestID: UUID, decision: ApprovalRequestDecision) throws {
            ...
            if approvalRequest.id == approvalRequestID, decision == .approve {
                ...
                hasApproved = true
                approvalRequest = SessionApprovalRequest(
                    id: approvalRequest.id, title: "Deploy?", text: "Deploy?", state: .approved)
            }
            let changeHandler = self.changeHandler
            lock.unlock()
            changeHandler?()
