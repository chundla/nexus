    ...
    struct NexusServiceRemoteIBMBobStructuredSessionTests {
        ...
        @Test func restartedRemoteIBMBobDefaultSessionStaysReadyAndResumableWithoutRemoteRuntimeRecovery() throws {
            ...
            #expect(
                restartedScreen.activityItems.map(\.text) == ["IBM Bob Session ready. Send a prompt to start IBM Bob."])
            ...
            #expect(
                resumedScreen.activityItems.map(\.text) == ["IBM Bob Session ready. Send a prompt to start IBM Bob."])
            #expect(transportHarness.launches().isEmpty)
        }
        ...
        @Test func remoteIBMBobNamedSessionUsesSharedNamingAndStructuredIdleScreen() throws {
            ...
            let secondSession = try service.createNamedSession(
        ...
        @Test func remoteIBMBobDefaultAndNamedSessionsResumeOnlyTheirOwnStoredContinuity() throws {
            ...
            let defaultSession = try service.launchOrResumeDefaultSession(
                workspaceID: workspace.id, providerID: .ibmBob)
            let reviewSession = try service.createNamedSession(
                workspaceID: workspace.id, providerID: .ibmBob, name: "Review")
            let followUpSession = try service.createNamedSession(
                workspaceID: workspace.id, providerID: .ibmBob, name: "Follow Up")
            ...
            let restartedService = try makeService()
            let followUpSecondResponse = try restartedService.sendSessionInput(
                sessionID: followUpSession.id, text: "follow up second")
            let defaultSecondResponse = try restartedService.sendSessionInput(
                sessionID: defaultSession.id, text: "default second")
            let reviewSecondResponse = try restartedService.sendSessionInput(
                sessionID: reviewSession.id, text: "review second")
            let launches = transportHarness.launches()
            ...
            #expect(
                followUpSecondResponse.activityItems.suffix(4).map(\.text) == [
            ...
            #expect(
                defaultSecondResponse.activityItems.suffix(4).map(\.text) == [
                    "You: default second",
            ...
            #expect(
                reviewSecondResponse.activityItems.suffix(4).map(\.text) == [
                    "You: review second",
        ...
        @Test func remoteIBMBobStopSessionInterruptsOnlyActiveTurnAndPreservesPartialHistory() throws {
            ...
            let service = try makeRemoteIBMBobService(
                rootURL: rootURL, transportFactory: transportHarness.makeTransport)
        ...
        @Test func remoteIBMBobBridgeLossLeavesInterruptedInspectableSessionUntilExplicitRelaunch() throws {
            ...
            #expect(relaunchedScreen.session.state == .ready)
            #expect(
                Array(relaunchedScreen.activityItems.map(\.text).prefix(4)) == partialScreen.activityItems.map(\.text))
        }
        ...
        @Test func remoteControllerSeesSharedRemoteIBMBobActivityAndMacCanReopenSameReadyHistory() throws {
            ...
            _ = try service.takeRemoteSessionControl(
                sessionID: session.id, pairedDeviceID: pairedDeviceID, columns: 44, rows: 12)
            let remoteScreen = try service.sendRemoteSessionInput(
                sessionID: session.id, pairedDeviceID: pairedDeviceID, text: "ship it")
            ...
            #expect(
                remoteScreen.activityItems.map(\.text) == [
                    "IBM Bob Session ready. Send a prompt to start IBM Bob.",
                    "You: ship it",
                    "Bob turn started",
                    "Hello from remote Bob",
    ...
    private func makeRemoteIBMBobService(rootURL: URL, transportHarness: RemoteIBMBobTransportHarness) throws
        -> NexusService
    {
        try makeRemoteIBMBobService(rootURL: rootURL, transportFactory: transportHarness.makeTransport)
    }

    private func makeRemoteIBMBobService(
        rootURL: URL, transportFactory: @escaping IBMBobSessionRuntime.TransportFactory
    ) throws -> NexusService {
    ...
    private final class RemoteIBMBobCommandRunner: ProviderCommandRunning, @unchecked Sendable {
        func run(executable: String, arguments: [String], currentDirectoryURL: URL?) throws -> ProviderCommandResult {
            guard let script = arguments.last else {
                throw NSError(
                    domain: "RemoteIBMBobCommandRunner", code: 1,
                    userInfo: [NSLocalizedDescriptionKey: "Missing SSH script for \(executable) \(arguments)"])
            }
            ...
            throw NSError(
                domain: "RemoteIBMBobCommandRunner", code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Unexpected SSH script: \(script)"])
        }
    }
    ...
    private final class RemoteIBMBobTransportHarness: @unchecked Sendable {
            ...
            -> any IBMBobTransporting
        {
            ...
            recordedLaunches.append(
                Launch(executable: executable, arguments: arguments, workingDirectory: workingDirectory))
