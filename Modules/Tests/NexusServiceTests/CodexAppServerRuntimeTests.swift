    ...
    private func codexTestTransportJSONLine(_ object: [String: Any]) -> String {
    ...
    struct CodexAppServerRuntimeTests {
        @Test func startsStructuredSessionAndCapturesCodexThreadLinkage() throws {
            ...
            #expect(
                runtime.sessionRecordAdapterMetadata
            ...
            #expect(
                transport.sentMessages.compactMap { $0["method"] as? String } == [
                    "initialize", "initialized", "thread/start", "model/list",
                ])
        }
        ...
        @Test func sessionScreenIncludesProviderEventsFromTheSharedCodexStream() throws {
            ...
            #expect(
                providerEvents.contains(where: {
                    $0.family == .response && $0.rawPayload.contains("\"model\":\"gpt-5.5\"")
                }))
        }
        ...
        @Test func resumesStructuredSessionFromExistingCodexThreadLinkage() throws {
            ...
            let resumeParameters = try #require(
                transport.sentMessages.first(where: { $0["method"] as? String == "thread/resume" })?["params"]
                    as? [String: Any]
            )

            #expect(
                transport.sentMessages.compactMap { $0["method"] as? String } == [
                    "initialize", "initialized", "thread/resume", "model/list",
                ])
        ...
        @Test func sendingPromptStartsCodexTurnAndSurfacesCompletedAgentMessage() throws {
            ...
            #expect(
                transport.sentMessages.compactMap { $0["method"] as? String } == [
                    "initialize", "initialized", "thread/start", "model/list", "turn/start",
                ])
        ...
        @Test func processCodexAppServerTransportLaunchesSiblingInterpreterForEnvShebangScripts() throws {
            ...
            try transport.sendLine(
                "{\"jsonrpc\":\"2.0\",\"id\":\"nexus-codex-readiness-initialize\",\"method\":\"initialize\",\"params\":{\"clientInfo\":{\"name\":\"nexus\",\"version\":\"1\"}}}"
            )
