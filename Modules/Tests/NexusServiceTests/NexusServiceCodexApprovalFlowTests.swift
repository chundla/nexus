...
struct NexusServiceCodexApprovalFlowTests {
    @Test func localCodexApprovalDecisionFlowsThroughSharedServiceContract() throws {
        ...
        #expect((transportHarness.transport.sentMessages.last?["result"] as? [String: String])?["decision"] == "accept")
    }
}
...
private struct CodexApprovalStubCommandRunner: ProviderCommandRunning {
    ...
    func run(executable: String, arguments: [String], currentDirectoryURL: URL?) throws -> ProviderCommandResult {
        guard let result = results[Invocation(executable: executable, arguments: arguments)] else {
            throw NSError(domain: "CodexApprovalStubCommandRunner", code: 1, userInfo: [NSLocalizedDescriptionKey: "Missing stub for \(executable) \(arguments)"])
        }
