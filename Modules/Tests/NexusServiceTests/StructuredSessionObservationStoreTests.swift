    ...
    struct StructuredSessionObservationStoreTests {
        @Test func structuredObservationSnapshotKeepsOnlyBoundedTranscriptTail() throws {
        ...
        @Test func structuredObservationDeltaKeepsOnlyBoundedTranscriptTail() throws {
            let store = StructuredSessionObservationStore()
            let session = Session(id: UUID(), workspaceID: UUID(), providerID: .pi, isDefault: true, state: .ready)
            ...
            case .structuredDelta(let value):
                delta = value
            default:
            ...
            let transcript = try #require(
                delta.changes.compactMap { change -> String? in
                    if case .setTranscript(let value) = change {
                        return value
                    }
                    return nil
                }.first)
        ...
        @Test func structuredObservationDeltaCarriesProviderFacts() throws {
            let store = StructuredSessionObservationStore()
            let session = Session(id: UUID(), workspaceID: UUID(), providerID: .pi, isDefault: true, state: .ready)
            _ = store.snapshotResponse(
                for: SessionScreen(
        ...
        @Test func structuredObservationDeltaReplacesSingleUpdatedActivityItem() throws {
            let store = StructuredSessionObservationStore()
            let session = Session(id: UUID(), workspaceID: UUID(), providerID: .pi, isDefault: true, state: .ready)
            let statusID = UUID()
            let commandID = UUID()
            ...
            let updatedScreen = SessionScreen(
                ...
                activityItems: [
                    SessionActivityItem(id: statusID, kind: .status, text: "Pi ready"),
                    SessionActivityItem(
                        id: commandID, kind: .command, text: "subagent reviewer", detailText: "step 1\nstep 2"),
                ]
            )
            ...
            #expect(
                delta.changes.contains {
                    if case .replaceActivityItem(let item) = $0 {
            ...
            #expect(
                delta.changes.contains {
                    if case .replaceActivityItemRange = $0 { return true }
                    return false
                } == false)
            #expect(
                delta.changes.contains {
                    if case .replaceActivityItems = $0 { return true }
                    return false
                } == false)
        }
