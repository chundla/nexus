    ...
    struct NexusMetadataStoreSessionRecordAdapterMetadataTests {
        ...
        @Test func oversizedPiActivityItemsAreCompactedBeforeMetadataPersistence() throws {
            ...
            let originalJSONCharacterCount = try #require(
                String(data: JSONEncoder().encode(activityItems), encoding: .utf8)?.count)
            ...
            let persistedPromptMessage = try #require(
                persistedActivityItems.first(where: { $0.text == "You: describe this image" }))
        ...
        @Test func reopeningStoreMigratesLegacyPiSessionLinkageIntoGenericSessionRecordMetadata() throws {
            ...
            let session: Session = try {
                ...
                let workspace = try store.createLocalWorkspace(
                    name: "Workspace",
                    folderPath: rootURL.appendingPathComponent("workspace", isDirectory: true).path(
            ...
            #expect(
                sqlite3_open_v2(storeURL.path, &database, SQLITE_OPEN_READWRITE | SQLITE_OPEN_FULLMUTEX, nil)
                    == SQLITE_OK)
            ...
            #expect(
                try reopenedStore.sessionRecordAdapterMetadata(sessionID: session.id)
