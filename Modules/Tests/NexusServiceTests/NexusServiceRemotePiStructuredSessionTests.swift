    ...
    struct NexusServiceRemotePiStructuredSessionTests {
        ...
        @Test func remotePiFailedNamedSessionAppearsInProviderDetailFailedSessionRecords() throws {
            ...
            let failedSession = try service.createNamedSession(
                workspaceID: workspace.id, providerID: .pi, name: "Review")
            let detail = try service.getProviderDetail(workspaceID: workspace.id, providerID: .pi)
        ...
        @Test func restartedRemotePiPendingExtensionDialogSurvivesReconnectWithProviderEvents() throws {
            ...
            #expect(
                interruptedScreen.providerEvents.contains(where: {
            ...
            #expect(
                resumedScreen.providerEvents.contains(where: {
                    $0.type == "extension_ui_request" && $0.rawPayload.contains("Deploy to production?")
                }))
    ...
    private final class RemotePiTransportHarness: @unchecked Sendable {
        ...
        private func sessionArgument(in arguments: [String]) -> String? {
            guard let command = arguments.last,
                let sessionFlagRange = command.range(of: "\"--session\" \"")
            else {
                return nil
            }
