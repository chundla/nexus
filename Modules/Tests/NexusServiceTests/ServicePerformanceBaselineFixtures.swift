...
struct ServicePerformanceBaselineFixtures {
    ...
    static func makeStructuredSessionFixture() throws -> SessionFixture {
        ...
        let service = try NexusService.bootstrapForTests(
            ...
            sessionRuntimeManager: InMemorySessionRuntimeManager(launcher: PerformanceBaselineStructuredRuntimeLauncher())
        )
        let group = try service.createWorkspaceGroup(name: "Solo Group")
        let workspace = try service.createLocalWorkspace(
            name: "Local Pi",
...
private struct PerformanceBaselineIBMBobProviderHealthFacts: ProviderHealthEvaluating {
    private let executableResolver = PerformanceBaselineIBMBobExecutableResolver(executables: ["bob": "/tmp/fake-bob"])
...
struct PerformanceBaselineReport {
    static func render(flow: String, record: PerformanceDiagnosticRecord) -> String {
        let steps = record.steps
            ...
            .joined(separator: ", ")
        let metrics = record.metrics.isEmpty
            ? "none"
            : record.metrics
                .sorted { $0.key < $1.key }
                .map { "\($0.key)=\($0.value)" }
        ...
        return "[Baseline] \(flow): operation=\(record.operation.rawValue) total=\(record.totalElapsedMilliseconds)ms steps=[\(steps)] metrics=[\(metrics)]"
    }
}
#endif
