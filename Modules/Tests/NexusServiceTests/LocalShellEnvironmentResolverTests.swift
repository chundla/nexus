...
private func makeShebangFixture() throws -> ShebangFixture {
    ...
    try "#!/bin/sh\nprintf '%s\\n' \"$NEXUS_TEST_ENV\"\n".write(to: interpreterPath, atomically: true, encoding: .utf8)
...
private func waitForTransportOutput<Status>(
    ...
    throw NSError(domain: "LocalShellEnvironmentResolverTests", code: 1, userInfo: [NSLocalizedDescriptionKey: "Timed out waiting for transport output."])
}
#endif
