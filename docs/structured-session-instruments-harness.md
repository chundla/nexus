...
Peak single-frame cost improved on run 4; **sustained** >33 ms/min is still far above run 1. Run **6** (~7.5 min, 2026-06-10) is the current long-session baseline in `214.trace`; startup red slightly better than run 5 but still above run 4 **296 ms**. After export, run **windowed** analysis: [baselines/214-trace-window-analysis.md](./baselines/214-trace-window-analysis.md) via `~/.pi/agent/skills/swiftui-expert-skill/scripts/analyze_trace.py` (`--window 0:30000` for #225, `--window 120000:400000` for #224 steady). If `xctrace export --toc` SIGSEGV on the multi-run bundle, export per run: `--run N` or xpath `run[@number="4"]` only.
